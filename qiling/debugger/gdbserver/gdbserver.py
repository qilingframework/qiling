...
class GDBSERVERsession(object):
    ...
    def __init__(self, ql, clientsocket, exit_point, mappings):
        ...
        if self.ql.ostype in (QL_LINUX, QL_FREEBSD):
            self.gdb.bp_insert(self.ql.os.loader.elf_entry)
        else:
    ...
    def run(self):
        ...
        while self.receive() == 'Good':
            ...
            def handle_c(subcmd):
                ...
                if self.gdb.bp_list in ([self.ql.os.loader.elf_entry], [self.ql.entry_point]):
                    self.send("W00")
                else:
            ...
            def handle_q(subcmd):
                if subcmd.startswith('Supported:'):
                    ...
                    if self.ql.ostype in (QL_LINUX, QL_FREEBSD):
                        ...
                        AT_HWCAP    = self.ql.addr_to_str(self.ql.os.loader.elf_hwcap)  # mock cpuid 0x1f8bfbff
                        AT_PAGESZ   = self.ql.addr_to_str(self.ql.os.loader.elf_pagesz)  # System page size, fixed in qiling
                        AT_PHDR     = self.ql.addr_to_str(self.ql.os.loader.elf_phdr)  # Program headers for program
                        AT_PHENT    = self.ql.addr_to_str(self.ql.os.loader.elf_phent)  # Size of program header entry
                        AT_PHNUM    = self.ql.addr_to_str(self.ql.os.loader.elf_phnum)  # Number of program headers
                        AT_BASE     = self.ql.addr_to_str(self.ql.os.loader.interp_base)  # Base address of interpreter
                        AT_FLAGS    = self.ql.addr_to_str(self.ql.os.loader.elf_flags)
                        AT_ENTRY    = self.ql.addr_to_str(self.ql.os.loader.elf_entry)  # Entry point of program
                        AT_UID      = self.ql.addr_to_str(self.ql.os.loader.elf_guid)  # UID at 1000 fixed in qiling
                        AT_EUID     = self.ql.addr_to_str(self.ql.os.loader.elf_guid)  # EUID at 1000 fixed in qiling
                        AT_GID      = self.ql.addr_to_str(self.ql.os.loader.elf_guid)  # GID at 1000 fixed in qiling
                        AT_EGID     = self.ql.addr_to_str(self.ql.os.loader.elf_guid)  # EGID at 1000 fixed in qiling
                        AT_RANDOM   = self.ql.addr_to_str(self.ql.os.loader.randstraddr)  # Address of 16 random bytes
                        AT_PLATFORM = self.ql.addr_to_str(self.ql.os.loader.cpustraddr)  # String identifying platform
