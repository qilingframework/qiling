        ...
        while self.receive() == "Good":
            ...
            def handle_qmark(subcmd):
                ...
                if self.ql.archtype == QL_ARCH.MIPS:
                    if self.ql.archendian == QL_ENDIAN.EB:
                        sp = self.addr_to_str(
                            self.ql.reg.arch_sp, endian="little"
                        )
                        pc = self.addr_to_str(
                            self.ql.reg.arch_pc, endian="little"
                        )
                    self.send(
                        "T%.2x%.2x:%s;%.2x:%s;"
                        % (GDB_SIGNAL_TRAP, idhex, sp, pcid, pc)
                    )
                else:
                    self.send(
                        "T%.2x%.2x:%s;%.2x:%s;%.2x:%s;"
                        % (GDB_SIGNAL_TRAP, idhex, nullfill, spid, sp, pcid, pc)
                    )
            ...
            def handle_G(subcmd):
                ...
                if self.ql.archtype == QL_ARCH.A8086:
                    for i in range(0, len(subcmd), 8):
                        ...
                        self.ql.reg.write(
                            self.tables[QL_ARCH.A8086][count], reg_data
                        )
                        count += 1
                    ...
                    for i in range(0, len(subcmd), 8):
                        ...
                        reg_data = int(reg_data, 16)
                        self.ql.reg.write(
                            self.tables[QL_ARCH.X86][count], reg_data
                        )
                        count += 1
                    ...
                    for i in range(0, 17 * 16, 16):
                        ...
                        reg_data = int(reg_data, 16)
                        self.ql.reg.write(
                            self.tables[QL_ARCH.X8664][count], reg_data
                        )
                        count += 1
                    ...
                    for i in range(0, len(subcmd), 8):
                        ...
                        reg_data = int(reg_data, 16)
                        self.ql.reg.write(
                            self.tables[QL_ARCH.ARM][count], reg_data
                        )
                        count += 1
                    ...
                    for i in range(0, len(subcmd), 16):
                        ...
                        reg_data = int(reg_data, 16)
                        self.ql.reg.write(
                            self.tables[QL_ARCH.ARM64][count], reg_data
                        )
                        count += 1
                    ...
                    for i in range(0, len(subcmd), 8):
                        ...
                        reg_data = int(reg_data, 16)
                        self.ql.reg.write(
                            self.tables[QL_ARCH.MIPS][count], reg_data
                        )
                        count += 1
            ...
            def handle_m(subcmd):
                ...
                try:
                    ...
                    for s in range(size):
                        ...
                        mem = "".join(
                            [
                                str("{:02x}".format(ord(c)))
                                for c in mem.decode("latin1")
                            ]
                        )
                        tmp += mem
                    self.send(tmp)
            ...
            def handle_p(subcmd):
                ...
                try:
                    if self.ql.archtype == QL_ARCH.A8086:
                        if reg_index <= 9:
                            reg_value = self.ql.reg.read(
                                self.tables[QL_ARCH.A8086][reg_index - 1]
                            )
                        else:
                            reg_value = 0
                        ...
                        if reg_index <= 24:
                            reg_value = self.ql.reg.read(
                                self.tables[QL_ARCH.X86][reg_index - 1]
                            )
                        else:
                            reg_value = 0
                        reg_value = self.addr_to_str(reg_value)
                        ...
                        if reg_index <= 32:
                            reg_value = self.ql.reg.read(
                                self.tables[QL_ARCH.X8664][reg_index - 1]
                            )
                        else:
                            reg_value = 0
                        if reg_index <= 17:
                        ...
                        if reg_index < 26:
                            reg_value = self.ql.reg.read(
                                self.tables[QL_ARCH.ARM][reg_index - 1]
                            )
                        else:
                            reg_value = 0
                        reg_value = self.addr_to_str(reg_value)
                        ...
                        if reg_index <= 32:
                            reg_value = self.ql.reg.read(
                                self.tables[QL_ARCH.ARM64][reg_index - 1]
                            )
                        else:
                            reg_value = 0
                            reg_value = self.addr_to_str(reg_value)
                        ...
                        if reg_index <= 37:
                            reg_value = self.ql.reg.read(
                                self.tables[QL_ARCH.MIPS][reg_index - 1]
                            )
                        else:
                            reg_value = 0
                        if self.ql.archendian == QL_ENDIAN.EL:
                            reg_value = self.addr_to_str(
                                reg_value, endian="little"
                            )
                        else:
                            reg_value = self.addr_to_str(reg_value)
            ...
            def handle_P(subcmd):
                ...
                if self.ql.archtype == QL_ARCH.A8086:
                    reg_data = int(reg_data, 16)
                    reg_data = int.from_bytes(
                        struct.pack("<I", reg_data), byteorder="big"
                    )
                    ...
                    if reg_index <= 17:
                        reg_data = int(reg_data, 16)
                        reg_data = int.from_bytes(
                            struct.pack("<Q", reg_data), byteorder="big"
                        )
                        self.ql.reg.write(reg_name, reg_data)
                    else:
                    ...
                    if self.ql.archendian == QL_ENDIAN.EL:
                        reg_data = int.from_bytes(
                            struct.pack("<I", reg_data), byteorder="little"
                        )
                    else:
                        reg_data = int.from_bytes(
                            struct.pack("<I", reg_data), byteorder="big"
                ...
                self.ql.log.info(
                    "gdb> Write to register %s with %x\n"
            ...
            def handle_q(subcmd):
                if subcmd.startswith("Supported:"):
                    ...
                    xfercmd_file = os.path.join(
                        xfercmd_abspath, "xml", xml_folder, xfercmd_file
                    )

                    if (
                        os.path.exists(xfercmd_file)
                        and self.ql.ostype is not QL_OS.WINDOWS
                    ):
                        ...
                        self.ql.log.info(
                            "gdb> Platform is not supported by xml or xml file not found: %s\n"
                            % (xfercmd_file)
                        )
                        self.send("l")
                    ...
                    if self.ql.ostype in (QL_OS.LINUX, QL_OS.FREEBSD):
                        if self.ql.archbit == 64:
                            ...
                            AT_CLKTCK = (
                                "6400000000000000"  # Frequency of times() 100
                            )
                            ...
                            AT_EXECFN = (
                                "0000000000000000"  # File name of executable
                            )
                            ...
                            ANNEX = "000000"
                            AT_SYSINFO_EHDR = (
                                "00000000"  # System-supplied DSO's ELF header
                            )
                        ...
                        AT_HWCAP = self.addr_to_str(
                            self.ql.loader.elf_hwcap
                        )  # mock cpuid 0x1f8bfbff
                        AT_PAGESZ = self.addr_to_str(
                            self.ql.loader.elf_pagesz
                        )  # System page size, fixed in qiling
                        AT_PHDR = self.addr_to_str(
                            self.ql.loader.elf_phdr
                        )  # Program headers for program
                        AT_PHENT = self.addr_to_str(
                            self.ql.loader.elf_phent
                        )  # Size of program header entry
                        AT_PHNUM = self.addr_to_str(
                            self.ql.loader.elf_phnum
                        )  # Number of program headers
                        AT_BASE = self.addr_to_str(
                            self.ql.loader.interp_address
                        )  # Base address of interpreter
                        AT_FLAGS = self.addr_to_str(self.ql.loader.elf_flags)
                        AT_ENTRY = self.addr_to_str(
                            self.ql.loader.elf_entry
                        )  # Entry point of program
                        AT_UID = self.addr_to_str(
                            self.ql.loader.elf_guid
                        )  # UID from ql.profile
                        AT_EUID = self.addr_to_str(
                            self.ql.loader.elf_guid
                        )  # UID from ql.profile
                        AT_GID = self.addr_to_str(
                            self.ql.loader.elf_guid
                        )  # UID from ql.profile
                        AT_EGID = self.addr_to_str(
                            self.ql.loader.elf_guid
                        )  # UID from ql.profile
                        AT_RANDOM = self.addr_to_str(
                            self.ql.loader.randstraddr
                        )  # Address of 16 random bytes
                        AT_PLATFORM = self.addr_to_str(
                            self.ql.loader.cpustraddr
                    ...
                    if self.ql.ostype in (QL_OS.LINUX, QL_OS.FREEBSD):
                        ...
                        self.send(
                            'l<library-list-svr4 version="1.0"></library-list-svr4>'
                        )
                    ...
                    self.send(
                        "T0;tnotrun:0;tframes:0;tcreated:0;tfree:50*!;tsize:50*!;circular:0;disconn:0;starttime:0;stoptime:0;username:;notes::"
                    )
            ...
            def handle_v(subcmd):
                ...
                if subcmd == "MustReplyEmpty":
                    ...
                    if (
                        self.ql.ostype == QL_OS.UEFI
                        and self.ql.custom_engine == True
                    ):
                        self.send("F-1")
                        return
                    ...
                    if file_path.startswith(self.rootfs_abspath):
                        ...
                        file_abspath = self.ql.os.path.transform_to_real_path(
                            file_path
                        )

                    self.ql.log.debug("gdb> target file: %s" % (file_abspath))
                    if os.path.exists(file_abspath) and not (
                        file_path
                    ).startswith("/proc"):
