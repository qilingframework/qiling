...
class QlGdbUtils(object):
    ...
    def initialize(self, ql, hook_address, exit_point=None, mappings=None):
        ...
        self._tmp_hook = self.ql.hook_address(
            self.entry_point_hook, hook_address
        )
    ...
    def dbg_hook(self, ql, address, size):
        ...
        try:
            ...
            if (
                (address & ~1) in self.bp_list
                and (address & ~1) != self.last_bp
            ) or self.has_soft_bp == True:
                if self.skip_bp_count > 0:
                    ...
                    ql.log.info(
                        "gdb> Breakpoint found, stop at address: 0x%x" % address
                    )
            ...
            if self.current_address + size == self.exit_point:
                ql.log.debug(
                    "gdb> emulation entrypoint at 0x%x" % (self.entry_point)
                )
                ql.log.debug(
                    "gdb> emulation exitpoint at 0x%x" % (self.exit_point)
                )

        except KeyboardInterrupt as ex:
            ql.log.info(
                "gdb> Paused at 0x%x, instruction size = %u" % (address, size)
            )
            self.ql.os.stop()
        except:
            raise
