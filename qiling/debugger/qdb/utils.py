...
def handle_bnj_arm(ql, cur_addr):
    ...
    if line.mnemonic in jump_table:
        ...
        to_jump = cb_table.get(line.mnemonic)(
            read_reg_val(line.op_str.split(", ")[0])
        )
    ...
    if to_jump:
        ...
        if regdst_eq_pc(line.op_str):
            ...
            if "]" in rn_offset.split(", ")[1]:  # pre-indexed immediate
                ...
                ret_addr = ql.unpack32(
                    ql.mem.read(parse_int(imm) + read_reg_val(r), 4)
                )
    ...
    elif line.mnemonic in ("addls", "addne", "add") and regdst_eq_pc(
        line.op_str
    ):
        ...
        if line.mnemonic == "addls" and (C == 0 or Z == 1):
            ...
            ret_addr = (
                8
                + read_reg_val(r1)
                + (read_reg_val(r2) * 4 if rest else read_reg_val(r2))
            )
        ...
        to_add = (
            int.from_bytes(
                ql.mem.read(cur_addr + r1, 2 if line.mnemonic == "tbh" else 1),
                byteorder="little",
            )
            * 2
        )
        ...
        ret_addr = ql.stack_read(
            line.op_str.strip("{").strip("}").split(", ").index("pc") * 4
        )
...
def handle_bnj_mips(ql, cur_addr):
    ...
    if line.mnemonic.startswith("j") or line.mnemonic.startswith("b"):
        ...
        targets = [
            read_reg_val(each) if "$" in each else parse_int(each)
        ...
        to_jump = {
            ...
            "ble": (
                lambda r0, r1, _: r0 <= r1
            ),  # brach on r0 less than or equal to r1
            "bge": (
                lambda r0, r1, _: r0 >= r1
            ),  # branch on r0 greater than or equal to r1
