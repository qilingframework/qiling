...
class QlReadELF(object):
    ...
    def elf_program_headers(self):
        ...
        for segment in self.elffile.iter_segments():
            ...
            program_hdr["Offset"] = self._format_hex(
                segment["p_offset"], fieldsize=6
            )
            program_hdr["VirtAddr"] = self._format_hex(
                segment["p_vaddr"], fullhex=True
            )
            program_hdr["PhysAddr"] = self._format_hex(
                segment["p_paddr"], fullhex=True
            )
            program_hdr["FileSiz"] = self._format_hex(
                segment["p_filesz"], fieldsize=5
            )
            program_hdr["MemSiz"] = self._format_hex(
                segment["p_memsz"], fieldsize=5
            )
    ...
    def elf_section_headers(self):
        ...
        for nsec, section in enumerate(self.elffile.iter_sections()):
            ...
            section_hdr["Addr"] = self._format_hex(
                section["sh_addr"], fieldsize=8, lead0x=False
            )
            section_hdr["Offset"] = self._format_hex(
                section["sh_offset"], fieldsize=6, lead0x=False
            )
            section_hdr["Size"] = self._format_hex(
                section["sh_size"], fieldsize=6, lead0x=False
            )
            section_hdr["ES"] = self._format_hex(
    ...
    def _symbol_version(self, nsym):
        ...
        if not index in ("VER_NDX_LOCAL", "VER_NDX_GLOBAL"):
            ...
            if (
                ...
                verneed, vernaux = self._versioninfo["verneed"].get_version(
                    index
                )
