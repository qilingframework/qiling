...
class Qiling(QlCoreHooks, QlCoreStructs):
    def __init__(
        ...
        self._uc = None
        self._stop_options = QlStopOptions(
            stackpointer=stop_on_stackpointer, exit_trap=stop_on_exit_trap
        )
        ...
        if self._code is None:
            if not os.path.exists(str(self._argv[0])):
                raise QlErrorFileNotFound(
                    "Target binary not found: %s" % (self._argv[0])
                )
        ...
        self._log_file_fd, self._log_filter = ql_setup_logger(
            self,
            self._log_file,
            self._console,
            self._filter,
    ...
    def enable_lib_patch(self):
        for addr, code, filename in self.patch_lib:
            try:
                ...
                raise RuntimeError(
                    "Fail to patch %s at address 0x%x" % (filename, addr)
                )
    ...
    def _init_stop_guard(self):
        ...
        self._exit_trap_addr = self.mem.find_free_space(
            0x1000, minaddr=0x9000000, align=0x10
        )
        ...
        if self.stop_options.stackpointer:
            ...
            def _check_sp(ql, address, size):
                if not ql.loader.skip_exit_check:
                    ...
                    if sp < 0:
                        self.log.info(
                            "Process returned from entrypoint (stackpointer)!"
                        )
                        ql.emu_stop()
    ...
    def write_exit_trap(self):
        ...
        if self.stop_options.any:
            if not self.loader.skip_exit_check:
                self.log.debug(
                    f"Setting up exit trap at 0x{hex(self._exit_trap_addr)}"
                )
                self.stack_write(0, self._exit_trap_addr)
            elif self.stop_options.exit_trap:
                self.log.debug(
                    f"Loader {self.loader} requested to skip exit_trap!"
                )
