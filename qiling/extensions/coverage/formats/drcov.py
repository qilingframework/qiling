...
class QlDrCoverage(QlBaseCoverage):
    ...
    def activate(self):
        self.bb_callback = self.ql.hook_block(
            self.block_callback, user_data=self
        )
    ...
    def dump_coverage(self, coverage_file):
        with open(coverage_file, "wb") as cov:
            ...
            cov.write(
                f"Module Table: version {self.drcov_version}, count {len(self.ql.loader.images)}\n".encode()
            )
            cov.write(
            ...
            for mod_id, mod in enumerate(self.ql.loader.images):
                cov.write(
                    f"{mod_id}, {mod.base}, {mod.end}, 0, 0, 0, {mod.path}\n".encode()
                )
