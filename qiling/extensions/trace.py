...
def __to_trace_line(
    record: TraceRecord, symsmap: Mapping[int, str] = {}
) -> str:
    ...
    state = tuple(
        (reg, val + insn.size if reg == UC_X86_REG_RIP else val)
    ...
    def __read_reg(reg: int) -> int:
        ...
        return (
            ...
            else next(v for r, v in state if r == reg)
        )
