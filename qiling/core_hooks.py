...
class QlCoreHooks:
    ...
    def _callback_type3(self, uc, intno, pack_data):
        (
            ...
            user_data,
            callback,
        ) = pack_data  # unpack what we packed for hook_add()
        if user_data:
        ...
        if hook_type in self._hook.keys():
            for h in self._hook[hook_type]:
                if h.check(ql, intno):
                    ...
                    ret = h.call(ql, intno)
                    if (
                        isinstance(ret, int) == True
                        and ret & QL_HOOK_BLOCK != 0
                    ):
                        break
    ...
    def _ql_hook_internal(self, hook_type, callback, user_data=None, *args):
        ...
        return self._h_uc.hook_add(
            hook_type, _callback, (self, user_data), 1, 0, *args
        )
    ...
    def _ql_hook_addr_internal(self, callback, user_data, address):
        ...
        # pack user_data & callback for wrapper _callback
        return self._h_uc.hook_add(
            UC_HOOK_CODE, _callback, (self, user_data), address, address
        )
            ...
            if (t & hook_type) != 0:
                if t in (UC_HOOK_INTR,):
                    if t not in self._hook_fuc.keys():
                        self._hook_fuc[t] = self._ql_hook_internal(
                            t, self._hook_intr_cb, t
                        )
                ...
                if t in (UC_HOOK_INSN,):
                    ...
                    if ins_t not in self._insn_hook_fuc.keys():
                        self._insn_hook_fuc[ins_t] = self._ql_hook_internal(
                            t, self._hook_insn_cb, ins_t, *args
                        )
                ...
                if t in (UC_HOOK_INSN_INVALID,):
                    if t not in self._hook_fuc.keys():
                        self._hook_fuc[t] = self._ql_hook_internal(
                            t, self._hook_insn_invalid_cb, t
                        )
    ...
    def ql_hook(
        self, hook_type, callback, user_data=None, begin=1, end=0, *args
    ):
    ...
    def hook_code(self, callback, user_data=None, begin=1, end=0):
        if self.custom_engine:
            ...
            return ql_engine_hooks(
                self, "ENGINE_HOOK_CODE", callback, user_data, begin, end
            )
    ...
    def hook_mem_unmapped(self, callback, user_data=None, begin=1, end=0):
        return self.ql_hook(
            UC_HOOK_MEM_UNMAPPED, callback, user_data, begin, end
        )

    def hook_mem_read_invalid(self, callback, user_data=None, begin=1, end=0):
        return self.ql_hook(
            UC_HOOK_MEM_READ_INVALID, callback, user_data, begin, end
        )

    def hook_mem_write_invalid(self, callback, user_data=None, begin=1, end=0):
        return self.ql_hook(
            UC_HOOK_MEM_WRITE_INVALID, callback, user_data, begin, end
        )

    def hook_mem_fetch_invalid(self, callback, user_data=None, begin=1, end=0):
        return self.ql_hook(
            UC_HOOK_MEM_FETCH_INVALID, callback, user_data, begin, end
        )
    ...
    def hook_mem_invalid(self, callback, user_data=None, begin=1, end=0):
        return self.ql_hook(
            UC_HOOK_MEM_INVALID, callback, user_data, begin, end
        )
    ...
    def hook_address(self, callback, address, user_data=None):
        ...
        if address not in self._addr_hook_fuc.keys():
            self._addr_hook_fuc[address] = self._ql_hook_addr_internal(
                self._hook_addr_cb, address, address
            )
    ...
    def hook_insn(self, callback, arg1, user_data=None, begin=1, end=0):
        if self.custom_engine:
            ...
            return engine_hook_insn(
                self, "ENGINE_HOOK_INSN", callback, arg1, user_data, begin, end
            )
        ...
        if isinstance(h, HookAddr):
            if h.addr in self._addr_hook.keys():
                if h in self._addr_hook[h.addr]:
                    del self._addr_hook[h.addr][
                        self._addr_hook[h.addr].index(h)
                    ]
