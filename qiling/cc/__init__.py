...
class QlCommonBaseCC(QlCC):
    ...
    def __access_param(
        self, index: int, stack_access: Callable, reg_access: Callable
    ) -> Tuple[Callable, int]:
        ...
        if index >= len(self._argregs):
            raise IndexError(
                f"tried to access arg {index}, but only {len(self._argregs) - 1} args are supported"
            )
    ...
    def getRawParam(self, index: int, argbits: int = None) -> int:
        read, loc = self.__access_param(
            index, self.ql.stack_read, self.ql.reg.read
        )
    ...
    def setRawParam(self, index: int, value: int, argbits: int = None) -> None:
        write, loc = self.__access_param(
            index, self.ql.stack_write, self.ql.reg.write
        )
