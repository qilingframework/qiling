...
class QlLoaderPE_UEFI(QlLoader):
    ...
    def map_and_load(self, path: str, exec_now: bool = False):
        ...
        assert (
            image_base % 0x1000
        ) == 0, "image base is expected to be page-aligned"
    ...
    def call_function(self, addr: int, args: Sequence[int], ret: Optional[int]):
        ...
        assert len(args) <= len(
            regs
        ), f"currently supporting up to {len(regs)} arguments"
    ...
    def unload_modules(self):
        for handle in self.loaded_image_protocol_modules:
            struct_addr = self.dxe_context.protocols[handle][
                self.loaded_image_protocol_guid
            ]
            loaded_image_protocol = (
                EfiLoadedImageProtocol.EFI_LOADED_IMAGE_PROTOCOL.loadFrom(
                    self.ql, struct_addr
                )
            )
            ...
            if unload_ptr != 0:
                self.ql.log.info(
                    f"Unloading module {handle:#x}, calling {unload_ptr:#x}"
                )
    ...
    def execute_next_module(self):
        if not self.modules or self.ql.os.notify_before_module_execution(
            self.ql, self.modules[0][0]
        ):
            return

        path, image_base, entry_point = self.modules.pop(0)
        self.execute_module(
            path, image_base, entry_point, self.end_of_execution_ptr
        )
    ...
    def run(self):
        ...
        arch_key = {QL_ARCH.X86: "OS32", QL_ARCH.X8664: "OS64"}[
            self.ql.archtype
        ]
