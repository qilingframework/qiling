...
from qiling.os.linux.kernel_api.kernel_api import (
    hook_sys_open,
    hook_sys_read,
    hook_sys_write,
)
...
class QlLoaderELF(QlLoader, ELFParse):
    ...
    def run(self):
        ...
        if not self.is_driver:
            self.load_with_ld(
                stack_address + stack_size, argv=self.argv, env=self.env
            )
        else:
    ...
    def load_with_ld(self, stack_addr, load_address=-1, argv=[], env={}):
        ...
        if elfhead["e_type"] == "ET_EXEC":
            ...
            self.ql.log.debug(
                "Some error in head e_type: %i!", elfhead["e_type"]
            )
            return -1
        ...
        loaddb = [
            dict(heappop(loadheap)[1].header) for i in range(len(loadheap))
        ]
        ...
        for entry in loaddb:
            ...
            if entry["p_type"] == "PT_INTERP":
                interp_path = self.NullStr(
                    super().getelfdata(entry["p_offset"], entry["p_filesz"])
                )
        ...
        for entry in loaddb:
            if entry["p_type"] == "PT_LOAD":
                _mem_start = (
                    (load_address + entry["p_vaddr"]) // pagesize
                ) * pagesize
                _mem_len = entry["p_memsz"]
                _mem_end = self.pcalc(
                    load_address + entry["p_vaddr"] + _mem_len, pagesize
                )
                _perms = self.translate_segment_perm_to_uc_prot(
                    entry["p_flags"]
                )
        ...
        for segment in _load_segments:
            ...
            try:
                self.ql.mem.map(
                    _mem_s, _mem_e - _mem_s, perms=_perms, info=self.path
                )
                ...
                self.ql.log.debug(
                    "load 0x%x - 0x%x => %s" % (_mem_s, _mem_e, str(e))
                )
                continue
        ...
        for entry in loaddb:
            if entry["p_type"] == "PT_LOAD" and entry["p_filesz"] > 0:
                try:
                    ...
                    data = super().getelfdata(
                        entry["p_offset"], entry["p_filesz"]
                    )
                    self.ql.mem.write(_mem_s, data)
                except Exception as e:
                    self.ql.log.debug(
                        "segment data 0x%x - Length 0x%x => %s"
                        % (_mem_s, len(data), str(e))
                    )
                    continue
        ...
        if loaded_mem_end > _mem_e:
            ...
            self.ql.log.debug(
                "load 0x%x - 0x%x" % (_mem_e, loaded_mem_end)
            )  # make sure we map all PT_LOAD tagged area

        entry_point = elfhead["e_entry"] + load_address
        self.ql.os.elf_mem_start = mem_start
        self.ql.log.debug(
            "mem_start: 0x%x mem_end: 0x%x" % (mem_start, mem_end)
        )
            ...
            for i in interp.parse_segments():
                ...
                if i["p_type"] == "PT_LOAD":
                    if (
                        ...
                        or interp_mem_size == -1
                    ):
                        interp_mem_size = i["p_vaddr"] + i["p_memsz"]

            interp_mem_size = (interp_mem_size // 0x1000 + 1) * 0x1000
            self.ql.log.debug(
                "interp_mem_size is : 0x%x" % int(interp_mem_size)
            )

            if self.ql.archbit == 64:
                self.interp_address = int(
                    self.ql.os.profile.get("OS64", "interp_address"), 16
                )
            elif self.ql.archbit == 32:
                self.interp_address = int(
                    self.ql.os.profile.get("OS32", "interp_address"), 16
                )

            self.ql.log.debug(
                "interp_address is : 0x%x" % (self.interp_address)
            )
            self.ql.mem.map(
                self.interp_address,
                int(interp_mem_size),
        ...
        # Set MMAP addr
        if self.ql.archbit == 64:
            self.mmap_address = int(
                self.ql.os.profile.get("OS64", "mmap_address"), 16
            )
        else:
            self.mmap_address = int(
                self.ql.os.profile.get("OS32", "mmap_address"), 16
            )
        ...
        # Set env
        if len(env) != 0:
            env_addr, new_stack = self.copy_str(
                new_stack, [key + "=" + value for key, value in env.items()]
            )
        ...
        self.ql.os.elf_entry = self.elf_entry = (
            load_address + elfhead["e_entry"]
        )
        self.stack_address = new_stack
        self.load_address = load_address
        self.images.append(
            Image(load_address, load_address + mem_end, self.path)
        )
        self.ql.os.function_hook = FunctionHook(
            self.ql,
            self.elf_phdr + mem_start,
            self.elf_phnum,
            self.elf_phent,
            load_address,
            load_address + mem_end,
        )
        ...
        if self.ql.archtype == QL_ARCH.X8664 and self.ql.ostype == QL_OS.LINUX:
            _vsyscall_addr = int(
                self.ql.os.profile.get("OS64", "vsyscall_address"), 16
            )
            _vsyscall_size = int(
                self.ql.os.profile.get("OS64", "vsyscall_size"), 16
            )
            ...
            if not self.ql.mem.is_mapped(_vsyscall_addr, _vsyscall_size):
                ...
                self.ql.mem.map(
                    _vsyscall_addr, _vsyscall_size, info="[vsyscall]"
                )
                ...
                _vsyscall_entry_asm = [
                    "mov rax, 0x60;",  # syscall gettimeofday
                ...
                for idx, val in enumerate(_vsyscall_entry_asm):
                    self.ql.mem.write(
                        _vsyscall_addr + idx * 0x400,
                        _compile(val + "; syscall; ret"),
                    )
            ...
            for nsym, symbol in enumerate(section.iter_symbols()):
                if symbol.name == "init_module":
                    addr = (
                        ...
                        + elffile.get_section(symbol["st_shndx"])["sh_offset"]
                    )
    ...
    def lkm_dynlinker(self, ql, mem_start):
        ...
        for section in _sections:
            ...
            for rel in section.iter_relocations():
                ...
                loc = (
                    elffile.get_section(section["sh_info"])["sh_offset"]
                    + rel["r_offset"]
                )
                loc += mem_start

                if describe_reloc_type(rel["r_info_type"], elffile) in (
                    "R_X86_64_32S",
                    "R_X86_64_32",
                ):
                    # patch this reloc
                    if rel["r_addend"]:
                        ...
                        ql.mem.write(
                            loc,
                            ql.pack32(
                                rev_reloc_symbols[symbol_name] & 0xFFFFFFFF
                            ),
                        )

                elif (
                    describe_reloc_type(rel["r_info_type"], elffile)
                    == "R_X86_64_64"
                ):
                    # patch this function?
                    val = sym_offset + rel["r_addend"]
                ...
                elif describe_reloc_type(rel["r_info_type"], elffile) in (
                    "R_X86_64_PC32",
                    "R_X86_64_PLT32",
                ):
                    ...
                    val = ql.unpack16(
                        ql.mem.read(prev_mips_hi16_loc + 2, 2)
                    ) << 16 | ql.unpack16(ql.mem.read(loc + 2, 2))
                    val = rev_reloc_symbols[symbol_name] + val
                    ...
                    raise QlErrorNotImplemented(
                        "Relocation type %s not implemented"
                        % describe_reloc_type(rel["r_info_type"], elffile)
                    )
    ...
    def load_driver(self, ql, stack_addr, loadbase=0):
        ...
        ql.log.info(
            "loadbase: %x, mem_start: %x, mem_end: %x"
            % (loadbase, mem_start, mem_end)
        )
        ...
        for sc in rev_reloc_symbols.keys():
            if sc != "sys_call_table" and sc.startswith("sys_"):
                ...
                if hasattr(SYSCALL_NR, tmp_sc):
                    ...
                    ql.log.debug(
                        "Writing syscall %s to [0x%x]"
                        % (sc, SYSCALL_MEM + ql.pointersize * syscall_id)
                    )
                    ql.mem.write(
                        SYSCALL_MEM + ql.pointersize * syscall_id,
                        ql.pack(rev_reloc_symbols[sc]),
                    )
        ...
        ql.mem.write(
            SYSCALL_MEM + 1 * ql.pointersize,
            ql.pack(self.ql.os.hook_addr + 1 * ql.pointersize),
        )
        # ql.mem.write(SYSCALL_MEM + 2  * 8, struct.pack("<Q", hook_sys_open))
        ql.mem.write(
            SYSCALL_MEM + 2 * ql.pointersize,
            ql.pack(self.ql.os.hook_addr + 2 * ql.pointersize),
        )

        # setup hooks for read/write/open syscalls
        self.import_symbols[self.ql.os.hook_addr] = hook_sys_read
        self.import_symbols[
            self.ql.os.hook_addr + 1 * ql.pointersize
        ] = hook_sys_write
        self.import_symbols[
            self.ql.os.hook_addr + 2 * ql.pointersize
        ] = hook_sys_open

    def get_elfdata_mapping(self):
        elfdata_mapping = bytearray()
        elfdata_mapping.extend(
            self.getelfdata(0, self.elfhead["e_ehsize"])
        )  # elf header
