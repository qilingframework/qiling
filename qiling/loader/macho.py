...
def load_commpage(ql):
    ...
    ql.mem.write(
        COMM_PAGE_START_ADDRESS + COMM_PAGE_CPUFAMILY, b"\xec\x5e\x3b\x57"
    )
    ql.mem.write(COMM_PAGE_START_ADDRESS + COMM_PAGE_KDEBUG_ENABLE, b"\x00")
    ql.mem.write(
        COMM_PAGE_START_ADDRESS + COMM_PAGE_ATM_DIAGNOSTIC_CONFIG, b"\x00"
    )
    ...
    ql.mem.write(
        COMM_PAGE_START_ADDRESS + COMM_PAGE_NT_GENERATION, b"\x01"
    )  # someflag seem important
    ...
    ql.mem.write(
        COMM_PAGE_START_ADDRESS + COMM_PAGE_APPROX_TIME_SUPPORTED, b"\x00"
    )
...
class QlLoaderMACHO(QlLoader):
    ...
    def __init__(self, ql, dyld_path=None):
        ...
        if os.path.isdir(self.ql.argv[0]):
            ...
            self.ql._argv = [
                self.ql.argv[0] + "/Contents/MacOS/" + self.kext_name
            ]
            self.ql._path = self.ql.argv[0]
            self.plist = plistlib.load(
                open(filename[0] + "/Contents/Info.plist", "rb")
            )
    ...
    def run(self):
        ...
        vmmap_trap_address = int(
            self.profile.get("OS64", "vmmap_trap_address"), 16
        )
        ...
        if self.ql.code:
            self.ql.mem.map(
                self.ql.os.entry_point,
                self.ql.os.code_ram_size,
                info="[shellcode_stack]",
            )
        ...
        self.ql.os.macho_port_manager = MachPortManager(
            self.ql, self.ql.os.macho_mach_port
        )
        ...
        self.ql.os.heap = QlMemoryHeap(
            self.ql, self.heap_address, self.heap_address + self.heap_size
        )
    ...
    def loadDriver(self, stack_addr, loadbase=-1, argv=[], env={}):
        ...
        kernel_path = os.path.join(
            self.ql.rootfs, "System/Library/Kernels/kernel.development"
        )
        ...
        self.vm_end_addr += PAGE_SIZE
        self.ql.log.info(
            "Memory for external static symbol is created at 0x%x with size 0x%x"
            % (self.static_addr, self.static_size)
        )
        self.static_symbols = {}
        ...
        for relocation in self.macho_file.dysymbol_table.locreloc:
            ...
            (current_value,) = struct.unpack(
                "<Q", self.ql.mem.read(loadbase + relocation.address, 8)
            )
            self.ql.log.debug(
                "Patching relocation (0x%x): from 0x%x, update to segment %s at 0x%x"
                % (
                    loadbase + relocation.address,
                    current_value,
                    seg.name,
                    loadbase + seg.vm_address,
                )
            )
            self.ql.mem.write(
                loadbase + relocation.address,
                struct.pack("<Q", current_value + loadbase),
            )
        ...
        self.kernel_local_symbols_detail = self.kernel.symbol_table.details(
            kernel_local_symbols_index,
            kernel_local_symbols_num,
            self.kernel.string_table,
        )
        ...
        self.kernel_extrn_symbols_detail = self.kernel.symbol_table.details(
            kernel_extrn_symbols_index,
            kernel_extrn_symbols_num,
            self.kernel.string_table,
        )
        ...
        for relocation in self.macho_file.dysymbol_table.extreloc:
            ...
            if relocation.length == 2 and relocation.rtype == 2:
                if symname not in self.static_symbols:
                    if symname in self.kernel_local_symbols_detail:
                        real_addr = self.kernel_local_symbols_detail[symname][
                            "n_value"
                        ]
                    elif b"_" + symname in self.kernel_extrn_symbols_detail:
                        # ___MALLOC ftw???
                        real_addr = self.kernel_extrn_symbols_detail[
                            b"_" + symname
                        ]["n_value"]
                    elif symname in self.kernel_extrn_symbols_detail:
                        real_addr = self.kernel_extrn_symbols_detail[symname][
                            "n_value"
                        ]
                    else:
                    ...
                    jmpcode = (
                        b"\x48\x83\xec\x08\xc7\x04\x24"
                        + struct.pack("<I", lo_addr)
                        + b"\xc7\x44\x24\x04"
                        + struct.pack("<I", hi_addr)
                        + b"\xc3"
                    )
            ...
            if symname in self.kernel_local_symbols_detail:
                ...
                self.ql.mem.write(
                    ...
                    struct.pack(
                        "<Q",
                        self.kernel_local_symbols_detail[symname]["n_value"],
                    ),
                )
            elif symname in self.kernel_extrn_symbols_detail:
                ...
                self.ql.mem.write(
                    ...
                    struct.pack(
                        "<Q",
                        self.kernel_extrn_symbols_detail[symname]["n_value"],
                    ),
                )
            else:
        ...
        self.kext_local_symbols = self.macho_file.symbol_table.details(
            index, num, self.macho_file.string_table
        )

        index = self.macho_file.dysymbol_table.defext_index
        num = self.macho_file.dysymbol_table.defext_num
        self.kext_extern_symbols = self.macho_file.symbol_table.details(
            index, num, self.macho_file.string_table
        )
        ...
        if self.IOKit is True:
            ...
            for symbol in self.macho_file.symbol_table.symbols:
                if (
                    symbol.n_type == 0xF
                    and "__const".ljust(16, "\x00")
                    == self.macho_file.sections[symbol.n_sect].name
                ):
                    symname = self.macho_file.string_table[symbol.n_strx]
                    self.ql.log.info(
                        "Found vtable of %s at 0x%x"
                        % (symname, loadbase + symbol.n_value)
                    )
                    self.vtables[symname] = loadbase + symbol.n_value

            kext = self.plist["IOKitPersonalities"][self.kext_name]["IOClass"]
            user = self.plist["IOKitPersonalities"][self.kext_name][
                "IOUserClientClass"
            ]
            ...
            for symname in self.vtables:
                ...
                if symname.decode().endswith(
                    str(len(kext)) + kext + "9MetaClassE"
                ):
                    (self.kext_alloc,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x98, 8)
                    )
                elif symname.decode().endswith(str(len(kext)) + kext):
                    (self.kext_init,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x258, 8)
                    )
                    (self.kext_attach,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x680, 8)
                    )
                    (self.kext_probe,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x5C8, 8)
                    )
                    (self.kext_detach,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x688, 8)
                    )
                    (self.kext_start,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x5D0, 8)
                    )
                elif symname.decode().endswith(
                    str(len(user)) + user + "9MetaClassE"
                ):
                    (self.user_alloc,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x98, 8)
                    )
                elif symname.decode().endswith(str(len(user)) + user):
                    (self.user_initWithTask,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x8F0, 8)
                    )
                    (self.user_attach,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x680, 8)
                    )
                    (self.user_start,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x5D0, 8)
                    )
                    (self.user_externalMethod,) = struct.unpack(
                        "<Q", self.ql.mem.read(self.vtables[symname] + 0x860, 8)
                    )
        else:
            ...
            if b"__realmain" in self.kext_local_symbols:
                realmain = (
                    loadbase + self.kext_local_symbols[b"__realmain"]["n_value"]
                )
                (current_value,) = struct.unpack(
                    "<Q", self.ql.mem.read(realmain, 8)
                )
            ...
            if b"__antimain" in self.kext_local_symbols:
                antimain = (
                    loadbase + self.kext_local_symbols[b"__antimain"]["n_value"]
                )
                (current_value,) = struct.unpack(
                    "<Q", self.ql.mem.read(antimain, 8)
                )
    ...
    def loadMacho(self, depth=0, isdyld=False):
                ...
                if pass_count == 3:
                    if cmd.cmd_id == LC_LOAD_DYLINKER:
                        ...
                        if not isdyld:
                            ...
                            self.dyld_path = os.path.join(
                                self.ql.rootfs + self.dyld_path
                            )
                            self.dyld_file = MachoParser(
                                self.ql, self.dyld_path
                            )
        ...
        if depth == 0:
            ...
            if self.using_dyld:
                ...
                self.ql.log.info(
                    "Dyld entry point: {}".format(hex(self.entry_point))
                )
            else:
                self.entry_point = self.proc_entry + self.slide
            self.ql.log.info(
                "Binary Entry Point: 0x{:X}".format(self.binary_entry)
            )
    ...
    def loadSegment64(self, cmd, isdyld):
        ...
        if seg_name[:10] == "__PAGEZERO":
            self.ql.log.debug(
                "Now loading {}, VM[{}:{}] for pagezero actually it only got a page size".format(
                    seg_name, hex(vaddr_start), hex(vaddr_end)
                )
            )
    ...
    def loadMain(self, cmd, isdyld=False):
        if self.macho_file.page_zero_size:
            if not isdyld:
                self.binary_entry = (
                    cmd.entry_offset + self.macho_file.page_zero_size
                )
    ...
    def push_stack_string(self, data):
        ...
        self.ql.log.debug(
            "SP {} write data len {}".format(hex(self.stack_sp), length)
        )
