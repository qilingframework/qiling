...
class EFI_SMM_BASE2_PROTOCOL(STRUCT):
    ...
    _fields_ = [
        (
            "InSmm",
            FUNCPTR(EFI_STATUS, PTR(EFI_SMM_BASE2_PROTOCOL), PTR(BOOLEAN)),
        ),
        (
            "GetSmstLocation",
            FUNCPTR(
                EFI_STATUS,
                PTR(EFI_SMM_BASE2_PROTOCOL),
                PTR(PTR(EFI_SMM_SYSTEM_TABLE2)),
            ),
        ),
    ]


@dxeapi(params={"This": POINTER, "InSmram": POINTER})
...
@dxeapi(params={"This": POINTER, "Smst": POINTER})
