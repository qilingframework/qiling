...
class EFI_SMM_SW_REGISTER_CONTEXT(STRUCT):
    _fields_ = [("SwSmiInputValue", UINTN)]


# @ file: MdePkg\Include\Pi\PiMmCis.h
EFI_SMM_HANDLER_ENTRY_POINT2 = FUNCPTR(
...
class EFI_SMM_SW_DISPATCH2_PROTOCOL(STRUCT):
    ...
    _fields_ = [
        (
            ...
            FUNCPTR(
                EFI_STATUS,
                PTR(EFI_SMM_SW_DISPATCH2_PROTOCOL),
                EFI_SMM_HANDLER_ENTRY_POINT2,
                PTR(EFI_SMM_SW_REGISTER_CONTEXT),
                PTR(EFI_HANDLE),
            ),
        ),
        (
            "UnRegister",
            FUNCPTR(EFI_STATUS, PTR(EFI_SMM_SW_DISPATCH2_PROTOCOL), EFI_HANDLE),
        ),
...
def hook_Register(ql: Qiling, address: int, params):
    ...
    SwRegisterContext = EFI_SMM_SW_REGISTER_CONTEXT.loadFrom(
        ql, RegisterContext
    )
    ...
    if idx == 0xFFFFFFFF:
        idx = next(
            (i for i in range(1, MAXIMUM_SWI_VALUE) if i not in handlers), None
        )
...
@dxeapi(params={"This": POINTER, "DispatchHandle": POINTER})
def hook_UnRegister(ql: Qiling, address: int, params):
    ...
    idx = next(
        (
            idx
            for idx, (_, args) in handlers.items()
            if args["DispatchHandle"] == DispatchHandle
        ),
        None,
    )
