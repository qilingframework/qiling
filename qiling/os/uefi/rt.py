...
@dxeapi(params={"Time": POINTER})  # IN PTR(EFI_TIME)
...
@dxeapi(params={"Enable": BOOL, "Time": POINTER})  # BOOLEAN  # PTR(EFI_TIME)
...
def hook_SetVariable(ql: Qiling, address: int, params):
    ql.env[params["VariableName"]] = bytes(
        ql.mem.read(params["Data"], params["DataSize"])
    )
    return EFI_SUCCESS


@dxeapi(params={"HighCount": POINTER})  # OUT PTR(UINT32)
