...
class EFI_TIMER_DELAY(ENUM):
    _members_ = ["TimerCancel", "TimerPeriodic", "TimerRelative"]


class EFI_INTERFACE_TYPE(ENUM):
    _members_ = ["EFI_NATIVE_INTERFACE"]
...
EFI_GET_WAKEUP_TIME = FUNCPTR(
    EFI_STATUS, PTR(BOOLEAN), PTR(BOOLEAN), PTR(EFI_TIME)
)
...
EFI_GET_VARIABLE = FUNCPTR(
    EFI_STATUS, PTR(CHAR16), PTR(EFI_GUID), PTR(UINT32), PTR(UINTN), PTR(VOID)
)
EFI_GET_NEXT_VARIABLE_NAME = FUNCPTR(
    EFI_STATUS, PTR(UINTN), PTR(CHAR16), PTR(EFI_GUID)
)
EFI_SET_VARIABLE = FUNCPTR(
    EFI_STATUS, PTR(CHAR16), PTR(EFI_GUID), UINT32, UINTN, PTR(VOID)
)
...
EFI_UPDATE_CAPSULE = FUNCPTR(
    EFI_STATUS, PTR(PTR(EFI_CAPSULE_HEADER)), UINTN, EFI_PHYSICAL_ADDRESS
)
EFI_QUERY_CAPSULE_CAPABILITIES = FUNCPTR(
    EFI_STATUS,
    PTR(PTR(EFI_CAPSULE_HEADER)),
    UINTN,
    PTR(UINT64),
    PTR(EFI_RESET_TYPE),
)
...
class EFI_DEVICE_PATH_PROTOCOL(STRUCT):
    _fields_ = [("Type", UINT8), ("SubType", UINT8), ("Length", UINT8 * 2)]
...
EFI_GET_MEMORY_MAP = FUNCPTR(
    EFI_STATUS,
    PTR(UINTN),
    PTR(EFI_MEMORY_DESCRIPTOR),
    PTR(UINTN),
    PTR(UINTN),
    PTR(UINT32),
)
...
EFI_CREATE_EVENT = FUNCPTR(
    EFI_STATUS, UINT32, EFI_TPL, EFI_EVENT_NOTIFY, PTR(VOID), PTR(EFI_EVENT)
)
...
EFI_INSTALL_PROTOCOL_INTERFACE = FUNCPTR(
    EFI_STATUS, PTR(EFI_HANDLE), PTR(EFI_GUID), EFI_INTERFACE_TYPE, PTR(VOID)
)
...
EFI_LOCATE_DEVICE_PATH = FUNCPTR(
    EFI_STATUS,
    PTR(EFI_GUID),
    PTR(PTR(EFI_DEVICE_PATH_PROTOCOL)),
    PTR(EFI_HANDLE),
)
EFI_INSTALL_CONFIGURATION_TABLE = FUNCPTR(EFI_STATUS, PTR(EFI_GUID), PTR(VOID))
EFI_IMAGE_LOAD = FUNCPTR(
    EFI_STATUS,
    BOOLEAN,
    EFI_HANDLE,
    PTR(EFI_DEVICE_PATH_PROTOCOL),
    PTR(VOID),
    UINTN,
    PTR(EFI_HANDLE),
)
...
EFI_CONNECT_CONTROLLER = FUNCPTR(
    EFI_STATUS,
    EFI_HANDLE,
    PTR(EFI_HANDLE),
    PTR(EFI_DEVICE_PATH_PROTOCOL),
...
EFI_OPEN_PROTOCOL_INFORMATION = FUNCPTR(
    EFI_STATUS,
    EFI_HANDLE,
    PTR(EFI_GUID),
    PTR(PTR(EFI_OPEN_PROTOCOL_INFORMATION_ENTRY)),
    PTR(UINTN),
)
EFI_PROTOCOLS_PER_HANDLE = FUNCPTR(
    EFI_STATUS, EFI_HANDLE, PTR(PTR(PTR(EFI_GUID))), PTR(UINTN)
)
EFI_LOCATE_HANDLE_BUFFER = FUNCPTR(
    EFI_STATUS,
    EFI_LOCATE_SEARCH_TYPE,
    PTR(EFI_GUID),
    PTR(VOID),
...
EFI_LOCATE_PROTOCOL = FUNCPTR(
    EFI_STATUS, PTR(EFI_GUID), PTR(VOID), PTR(PTR(VOID))
)
EFI_INSTALL_MULTIPLE_PROTOCOL_INTERFACES = FUNCPTR(
    EFI_STATUS, PTR(EFI_HANDLE)
)  # ...
EFI_UNINSTALL_MULTIPLE_PROTOCOL_INTERFACES = FUNCPTR(
