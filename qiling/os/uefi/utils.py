...
def execute_protocol_notifications(ql: Qiling, from_hook: bool = False) -> bool:
    ...
    def __notify_next(ql: Qiling):
        if ql.loader.notify_list:
            ...
            ql.log.info(
                f'Notify event: id = {event_id}, (*{notify_func:#x})({", ".join(f"{a:#x}" for a in callback_args)})'
            )
...
def check_and_notify_protocols(ql: Qiling, from_hook: bool = False) -> bool:
    if ql.loader.notify_list:
        ...
        ql.log.info(
            f"Notify event: id = {event_id}, calling: {notify_func:#x} context: {notify_context}"
        )
        ...
        if from_hook:
            ...
            ql.loader.call_function(
                notify_func, notify_context, ql.loader.end_of_execution_ptr
            )
...
def GetEfiConfigurationTable(context, guid: str) -> Optional[int]:
    ...
    if guid in confs:
        ...
        ptr = context.conf_table_array_ptr + (
            idx * EFI_CONFIGURATION_TABLE.sizeof()
        )
