...
class EFI_MM_IO_WIDTH(ENUM):
    _members_ = ["MM_IO_UINT8", "MM_IO_UINT16", "MM_IO_UINT32", "MM_IO_UINT64"]


EFI_MM_HANDLER_ENTRY_POINT = FUNCPTR(
    EFI_STATUS, EFI_HANDLE, PTR(VOID), PTR(VOID), PTR(UINTN)
)
...
class EFI_SMM_CPU_IO2_PROTOCOL(STRUCT):
    _fields_ = [("Mem", EFI_MM_IO_ACCESS), ("Io", EFI_MM_IO_ACCESS)]
...
class EFI_SMM_SYSTEM_TABLE2(STRUCT):
    ...
    _fields_ = [
        ...
        (
            "SmmAllocatePool",
            FUNCPTR(EFI_STATUS, EFI_MEMORY_TYPE, UINTN, PTR(PTR(VOID))),
        ),
        ...
        (
            ...
            FUNCPTR(
                ...
                EFI_MEMORY_TYPE,
                UINTN,
                PTR(EFI_PHYSICAL_ADDRESS),
            ),
        ),
        ("SmmFreePages", FUNCPTR(EFI_STATUS, EFI_PHYSICAL_ADDRESS, UINTN)),
        (
            "SmmStartupThisAp",
            FUNCPTR(EFI_STATUS, FUNCPTR(VOID, PTR(VOID)), UINTN, PTR(VOID)),
        ),
        ...
        (
            ...
            FUNCPTR(EFI_STATUS, PTR(VOID), PTR(EFI_GUID), PTR(VOID)),
        ),
        (
            "SmmHandleProtocol",
            FUNCPTR(EFI_STATUS, PTR(VOID), PTR(EFI_GUID), PTR(PTR(VOID))),
        ),
        (
            "SmmRegisterProtocolNotify",
            FUNCPTR(
                EFI_STATUS, PTR(EFI_GUID), EFI_MM_NOTIFY_FN, PTR(PTR(VOID))
            ),
        ),
        (
            "SmmLocateHandle",
        ...
        (
            "SmmLocateProtocol",
            FUNCPTR(EFI_STATUS, PTR(EFI_GUID), PTR(VOID), PTR(PTR(VOID))),
        ),
        (
            "SmiManage",
            FUNCPTR(
                EFI_STATUS, PTR(EFI_GUID), PTR(VOID), PTR(VOID), PTR(UINTN)
            ),
        ),
        (
            "SmiHandlerRegister",
...
@dxeapi(params={"Guid": GUID, "Table": POINTER})  # PTR(EFI_GUID)  # PTR(VOID)
...
@dxeapi(params={"Buffer": POINTER})  # PTR(VOID)
...
@dxeapi(
    params={"Procedure": POINTER, "CpuNumber": INT, "ProcArguments": POINTER}
)
...
@dxeapi(
    params={"Handler": POINTER, "HandlerType": GUID, "DispatchHandle": POINTER}
)
def hook_SmiHandlerRegister(ql, address, params):
    return EFI_SUCCESS


@dxeapi(params={"DispatchHandle": POINTER})
...
def initialize(ql, gSmst: int):
    ...
    descriptor = {
        ...
        "fields": (
            ...
            (
                "SmmUninstallProtocolInterface",
                hook_SmmUninstallProtocolInterface,
            ),
    ...
    install_configuration_table(
        ql.loader.smm_context, "SMM_RUNTIME_SERVICES_TABLE", gSmmRT
    )


__all__ = ["EFI_SMM_SYSTEM_TABLE2", "initialize"]
