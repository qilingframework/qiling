...
class QlOsUefi(QlOs):
    ...
    def process_fcall_params(
        ...
        def fallback(v):
            ...
            # only need the value
            return super(QlOsUefi, self).process_fcall_params([(None, "", v)])[
                0
            ][1]
        ...
        return tuple(
            (aname, ahandlers.get(atype, fallback)(avalue))
    ...
    def emit_context(self):
        ...
        for grp in rgroups:
            self.ql.log.error(
                ", ".join(
                    (f"{reg:4s} = {self.ql.reg.read(reg):0{bits // 4}x}")
    ...
    def emit_hexdump(self, address: int, data: bytearray, num_cols: int = 16):
        ...
        for i in range(0, len(chars), num_cols):
            hexdump = " ".join(
                f"  " if ch is None else f"{ch:02x}"
                for ch in chars[i : i + num_cols]
            )
    ...
    def emit_disasm(self, address: int, data: bytearray, num_insns: int = 8):
        ...
        for insn in tuple(md.disasm(data, address))[:num_insns]:
            ...
            self.ql.log.error(
                f"{insn.address:08x} : {opcodes:28s}  {insn.mnemonic:10s} {insn.op_str:s}"
            )
    ...
    def emu_error(self):
        ...
        try:
            ...
            img_info = (
                f" ({containing_image.path} + {pc - containing_image.base:#x})"
                if containing_image
                else ""
            )
