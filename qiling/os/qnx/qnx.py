...
class QlOsQnx(QlOsPosix):
    ...
    def run(self):
        ...
        self.cpupage_addr = int(
            self.ql.os.profile.get("OS32", "cpupage_address"), 16
        )
        self.cpupage_tls_addr = int(
            self.ql.os.profile.get("OS32", "cpupage_tls_address"), 16
        )
        self.tls_data_addr = int(
            self.ql.os.profile.get("OS32", "tls_data_address"), 16
        )

        self.syspage_addr = int(
            self.ql.os.profile.get("OS32", "syspage_address"), 16
        )
        ...
        self.ql.mem.write(
            self.cpupage_addr, self.ql.pack32(self.cpupage_tls_addr)
        )
        ...
        self.ql.mem.write(
            self.cpupage_addr + 8, self.ql.pack32(self.syspage_addr)
        )
        ...
        try:
            if self.ql.code:
                self.ql.emu_start(
                    self.entry_point,
                    (self.entry_point + len(self.ql.code)),
                    self.ql.timeout,
                    self.ql.count,
                )
            else:
