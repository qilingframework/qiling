...
def ql_qnx_msg_mem_map(ql: Qiling, coid, smsg, sparts, rmsg, rparts, *args, **kw):
    ...
    ql.log.debug(f'mem_map(addr = 0x{addr:x}, len = 0x{len:x}, prot = {qnx_mmap_prot_flags(prot)}, flags = {_flags_mapping(flags, mmap_flags)}, fd = {fd}, preload = 0x{preload:x}, align = 0x{align:x}, offset = 0x{offset:x})')
    # map memory
