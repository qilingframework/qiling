...
def ql_syscall_clock_gettime(
    ql, clock_gettime_clock_id, clock_gettime_timespec, *args, **kw
):
    ql.log.info(
        f"clock_gettime(clock_id={clock_gettime_clock_id}, tp={hex(clock_gettime_timespec)})"
    )
...
def ql_syscall___sysctl(ql, name, namelen, old, oldlenp, new_arg, newlen):
    ql.log.debug(
        "__sysctl(name: 0x%x, namelen: 0x%x, old: 0x%x, oldlenp: 0x%x, new: 0x%x, newlen: 0x%x)"
