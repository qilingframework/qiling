...
def ql_syscall_ugetrlimit(
    ql, ugetrlimit_resource, ugetrlimit_rlim, *args, **kw
):
...
def ql_syscall_getrlimit(ql, getrlimit_resource, getrlimit_rlim, *args, **kw):
    return ql_syscall_ugetrlimit(
        ql, getrlimit_resource, getrlimit_rlim, *args, **kw
    )
...
def ql_syscall_setrlimit(ql, setrlimit_resource, setrlimit_rlim, *args, **kw):
    ...
    tmp_rlim = (
        ql.unpack32s(ql.mem.read(setrlimit_rlim, 4)),
        ql.unpack32s(ql.mem.read(setrlimit_rlim + 4, 4)),
    )
...
def ql_syscall_getpriority(
    ql, getpriority_which, getpriority_who, null1, null2, null3, null4
):
