...
def ql_syscall_madvise(
    ql, madvise_addr, madvise_length, madvise_advice, *args, **kw
):
    MADV_DONTNEED = 4
...
def ql_syscall_mprotect(ql, start, mlen, prot, *args, **kw):
    regreturn = 0
    ql.log.debug(
        "mprotect(0x%x, 0x%x, %s) = %d"
        % (start, mlen, mmap_prot_mapping(prot), regreturn)
    )
    ...
    try:
        ...
        raise QlMemoryMappedError(
            "Error: change protection at: %x - %x" % (start, start + mlen - 1)
        )
...
def syscall_mmap_impl(ql, addr, mlen, prot, flags, fd, pgoffset, ver):
    ...
    ql.log.debug(
        ...
        % (
            ...
            addr,
            mlen,
            mmap_prot_mapping(prot),
            prot,
            mmap_flag_mapping(flags),
            flags,
            fd,
            pgoffset,
        )
    ...
    if need_mmap:
        ...
        try:
            ql.mem.map(
                mmap_base, eff_mmap_size, info=("[syscall_%s]" % api_name)
            )
        except Exception as e:
            raise QlMemoryMappedError("Error: mapping needed but failed")

    ql.log.debug(
        "%s - addr range  0x%x - 0x%x: "
        % (api_name, mmap_base, mmap_base + eff_mmap_size - 1)
    )
    ...
    ql.log.debug(
        "%s(0x%x, 0x%x, 0x%x, 0x%x, %x, 0x%x) = 0x%x"
...
def ql_syscall_mmap(
    ql, mmap_addr, mmap_length, mmap_prot, mmap_flags, mmap_fd, mmap_pgoffset
):
    return syscall_mmap_impl(
        ql,
        mmap_addr,
