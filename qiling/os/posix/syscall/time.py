...
def ql_syscall_nanosleep(ql, nanosleep_req, nanosleep_rem, *args, **kw):
    ...
    regreturn = 0
    ql.log.info("nanosleep(0x%x, 0x%x) = %d" % (nanosleep_req, nanosleep_rem, regreturn))
    return regreturn
...
def ql_syscall_setitimer(ql, setitimer_which, setitimer_new_value, setitimer_old_value, *args, **kw):
    ...
    regreturn = 0
    ql.log.info("setitimer(%d, %x, %x) = %d" % (setitimer_which, setitimer_new_value, setitimer_old_value, regreturn))
    return regreturn
...
def ql_syscall_times(ql, times_tbuf, *args, **kw):
    ...
    ql.log.info('times(%x) = %d' % (times_tbuf, regreturn))
    return regreturn
...
def ql_syscall_gettimeofday(ql, gettimeofday_tv, gettimeofday_tz, *args, **kw):
    ...
    regreturn = 0
    ql.log.info("gettimeofday(%x, %x) = %d" % (gettimeofday_tv, gettimeofday_tz, regreturn))
    return regreturn
