...
def ql_syscall_faccessat(ql, faccessat_dfd, faccessat_filename, faccessat_mode, *args, **kw):
    ...
    real_path = ql.os.utils.transform_to_real_path(access_path)
    relative_path = ql.os.utils.transform_to_relative_path(access_path)
...
def ql_syscall_readlink(ql, path_name, path_buff, path_buffsize, *args, **kw):
    ...
    real_path = ql.os.utils.transform_to_link_path(pathname)
    relative_path = ql.os.utils.transform_to_relative_path(pathname)
...
def ql_syscall_getcwd(ql, path_buff, path_buffsize, *args, **kw):
    localpath = ql.os.utils.transform_to_relative_path('./')
    localpath = bytes(localpath, 'utf-8') + b'\x00'
    ql.mem.write(path_buff, localpath)
