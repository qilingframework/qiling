...
def ql_syscall_socket(
    ql, socket_domain, socket_type, socket_protocol, *args, **kw
):
    ...
    ql.log.debug(
        "socket(%s, %s, %s) = %d"
        % (socket_domain, socket_type, socket_protocol, regreturn)
    )

    return regreturn


def ql_syscall_connect(
    ql, connect_sockfd, connect_addr, connect_addrlen, *args, **kw
):
    AF_UNIX = 1
    AF_INET = 2
...
def ql_syscall_getsockopt(
    ql, sockfd, level, optname, optval_addr, optlen_addr, *args, **kw
):
...
def ql_syscall_setsockopt(
    ql, sockfd, level, optname, optval_addr, optlen, *args, **kw
):
    if not (0 <= sockfd < NR_OPEN) or ql.os.fd[sockfd] == 0:
        return -EBADF
...
def ql_syscall_bind(ql, bind_fd, bind_addr, bind_addrlen, *args, **kw):
    ...
    # IPv4 should comes first
        ...
        ql.os.ipv6 == True
        and sin_family == 10
        and ql.os.bindtolocalhost == True
    ):
    ...
    if sin_family == 1:
        ql.log.debug(
            "bind(%d, %s, %d) = %d" % (bind_fd, path, bind_addrlen, regreturn)
        )
    else:
        ql.log.debug(
            "bind(%d,%s:%d,%d) = %d"
            % (bind_fd, host, port, bind_addrlen, regreturn)
        )
        ql.log.debug(
            "syscall bind host: %s and port: %i sin_family: %i"
            % (ql_bin_to_ip(host), port, sin_family)
        )
...
def ql_syscall_getsockname(ql, sockfd, addr, addrlenptr, *args, **kw):
    ...
    ql.log.debug(
        "getsockname(%d, 0x%x, 0x%x) = %d"
        % (sockfd, addr, addrlenptr, regreturn)
    )
    return regreturn
...
def ql_syscall_accept(
    ql, accept_sockfd, accept_addr, accept_addrlen, *args, **kw
):
    def inet_addr(ip):
        ret = b""
        tmp = ip.split(".")
...
def ql_syscall_recv(
    ql, recv_sockfd, recv_buf, recv_len, recv_flags, *args, **kw
):
...
def ql_syscall_send(
    ql, send_sockfd, send_buf, send_len, send_flags, *args, **kw
):
    regreturn = 0
...
def ql_syscall_recvmsg(ql, sockfd, msg_addr, flags, *args, **kw):
    ...
    if 0 <= sockfd < NR_OPEN and ql.os.fd[sockfd] != 0:
        ...
        try:
            data, ancdata, mflags, addr = ql.os.fd[sockfd].recvmsg(
                msg.msg_namelen, msg.msg_controllen, flags
            )
            ...
            for i in range(msg.msg_iovlen):
                ...
                ql.mem.write(
                    vec.iov_base, data[has_written : has_written + size]
                )
    ...
    **kw
    ...
    if ql.os.fd[recvfrom_sockfd].socktype == SOCK_STREAM:
        ...
        if 0 <= recvfrom_sockfd < NR_OPEN and ql.os.fd[recvfrom_sockfd] != 0:
            tmp_buf, tmp_addr = ql.os.fd[recvfrom_sockfd].recvfrom(
                recvfrom_len, recvfrom_flags
            )
            if tmp_buf:
            ...
            if sin_family == 1:
                ...
                ql.log.debug(
                    "recvfrom() addr is %s:%d" % (tmp_addr[0], tmp_addr[1])
                )
                data += struct.pack(">H", tmp_addr[1])
                data += ipaddress.ip_address(tmp_addr[0]).packed
                addrlen = ql.unpack(
                    ql.mem.read(recvfrom_addrlen, ql.pointersize)
                )
                data = data[:addrlen]
    ...
    **kw
    ...
    if ql.os.fd[sendto_sockfd].socktype == SOCK_STREAM:
        return ql_syscall_send(
            ql, sendto_sockfd, sendto_buf, sendto_len, sendto_flags, *args, **kw
        )
    else:
        regreturn = 0
        if 0 <= sendto_sockfd < NR_OPEN and ql.os.fd[sendto_sockfd] != 0:
            try:
                ...
                if sin_family == 1:
                    ...
                    regreturn = ql.os.fd[sendto_sockfd].sendto(
                        bytes(tmp_buf), sendto_flags, path
                    )
                else:
                    ql.log.debug("sendto() addr is %s:%d" % (host, port))
                    regreturn = ql.os.fd[sendto_sockfd].sendto(
                        bytes(tmp_buf), sendto_flags, (host, port)
                    )
