...
class QlOsLinux(QlOsPosix):
    ...
    def load(self):
        ...
        stacksize = int(self.profile.get("OS", "stack_size"),16)

        # ARM
        if self.ql.archtype== QL_ARCH.ARM:
            stackaddress = int(self.profile.get("ARM", "stack_address"),16)
        ...
        # MIPS32
            ...
            stackaddress = int(self.profile.get("MIPS", "stack_address"),16)
            stacksize = int(self.profile.get("MIPS", "stack_size"),16)        
            self.ql.hook_intno(self.hook_syscall, 17)
            self.thread_class = QlLinuxMIPS32Thread

        # ARM64
        elif self.ql.archtype== QL_ARCH.ARM64:
            stackaddress = int(self.profile.get("ARM64", "stack_address"),16)
            self.ql.arch.enable_vfp()
            self.ql.hook_intno(self.hook_syscall, 2)
        ...
        # X86
            ...
            stackaddress = int(self.profile.get("X86", "stack_address"),16)
        ...
        # X8664
            ...
            stackaddress = int(self.profile.get("X8664", "stack_address"),16)
            self.gdtm = GDTManager(self.ql)
            ql_x86_register_cs(self)
            ql_x86_register_ds_ss_es(self)
