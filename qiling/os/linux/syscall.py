...
def ql_syscall_set_thread_area(ql: Qiling, u_info_addr, *args, **kw):
    if ql.archtype == QL_ARCH.X86:
        ...
        ql.log.debug(
            "set_thread_area base : 0x%x limit is : 0x%x" % (base, limit)
        )
        ...
        if (
            index == -1
            or index < GDT_ENTRY_TLS_MIN
            or index > GDT_ENTRY_TLS_MAX
        ):
            ql.log.warning(
                f"Wrong index {index} from address {hex(u_info_addr)}"
            )
            return -1
        else:
            ql.os.gdtm.register_gdt_segment(
                index,
                base,
                limit,
                QL_X86_A_PRESENT
