...
class QlLinuxThread(QlThread):
    ...
    def _run(self):
        ...
        while self.status != THREAD_STATUS_TERMINATED:
            ...
            if (
                ...
                or self.ql.reg.arch_pc == self.exit_point
            ):
                break
    ...
    def clone(self):
        ...
        new_thread = self.ql.os.thread_class.spawn(
            self._ql,
            self._start_address,
            self._exit_point,
            self._saved_context,
    ...
    def _on_stop(self):
        ...
        if self.clear_child_tid_address is not None:
            self.ql.log.debug(
                f"Perform CLONE_CHILD_CLEARTID at {hex(self.clear_child_tid_address)}"
            )
            self.ql.mem.write(self.clear_child_tid_address, self.ql.pack32(0))
            wakes = self.ql.os.futexm.get_futex_wake_list(
                self.ql, self.clear_child_tid_address, 1
            )
...
class QlLinuxX86Thread(QlLinuxThread):
    ...
    def __init__(
        ...
        super(QlLinuxX86Thread, self).__init__(
            ql,
            start_address,
            exit_point,
            context,
    ...
    def set_thread_tls(self, tls_addr):
        ...
        if index == -1 or index < 12 or index > 14:
            ...
            self.ql.os.gdtm.register_gdt_segment(
                index,
                base,
                limit,
                QL_X86_A_PRESENT
        ...
        self.ql.log.debug(
            f"Set tls to index={hex(index)} base={hex(base)} limit={hex(limit)} fs={hex(self.ql.reg.fs)} gs={hex(self.ql.reg.gs)} gdt_buf={self.tls}"
        )
    ...
    def save(self):
        ...
        self.tls = bytes(self.ql.os.gdtm.get_gdt_buf(12, 14 + 1))
        self.ql.log.debug(
            f"Saved context. fs={hex(self.ql.reg.fs)} gs={hex(self.ql.reg.gs)} gdt_buf={self.tls}"
        )
    ...
    def restore(self):
        ...
        self.ql.os.gdtm.set_gdt_buf(12, 14 + 1, self.tls)
        self.ql.log.debug(
            f"Restored context. fs={hex(self.ql.reg.fs)} gs={hex(self.ql.reg.gs)} gdt_buf={self.tls}"
        )
...
class QlLinuxX8664Thread(QlLinuxThread):
    ...
    def __init__(
        ...
        super(QlLinuxX8664Thread, self).__init__(
            ql,
            start_address,
            exit_point,
            context,
    ...
    def save(self):
        ...
        self.ql.log.debug(
            f"Saved context: fs={hex(self.ql.reg.fsbase)} tls={hex(self.tls)}"
        )
    ...
    def restore(self):
        ...
        self.ql.log.debug(
            f"Restored context: fs={hex(self.ql.reg.fsbase)} tls={hex(self.tls)}"
        )
...
class QlLinuxMIPS32Thread(QlLinuxThread):
    ...
    def __init__(
        ...
        super(QlLinuxMIPS32Thread, self).__init__(
            ql,
            start_address,
            exit_point,
            context,
    ...
    def save(self):
        ...
        self.ql.log.debug(
            f"Saved context. cp0={hex(self.ql.reg.cp0_userlocal)}"
        )
    ...
    def restore(self):
        ...
        self.set_thread_tls(self.tls)
        self.ql.log.debug(
            f"Restored context. cp0={hex(self.ql.reg.cp0_userlocal)}"
        )
...
class QlLinuxARMThread(QlLinuxThread):
    ...
    def __init__(
        ...
        super(QlLinuxARMThread, self).__init__(
            ql,
            start_address,
            exit_point,
            context,
    ...
    def save(self):
        ...
        self.ql.log.debug(
            f"Saved context. c13_c0_3={hex(self.ql.reg.c13_c0_3)}"
        )
    ...
    def restore(self):
        ...
        self.set_thread_tls(self.tls)
        self.ql.log.debug(
            f"Restored context. c13_c0_3={hex(self.ql.reg.c13_c0_3)}"
        )
...
class QlLinuxARM64Thread(QlLinuxThread):
    ...
    def __init__(
        ...
        super(QlLinuxARM64Thread, self).__init__(
            ql,
            start_address,
            exit_point,
            context,
    ...
    def save(self):
        ...
        self.ql.log.debug(
            f"Saved context. tpidr_el0={hex(self.ql.reg.tpidr_el0)}"
        )
    ...
    def restore(self):
        ...
        self.set_thread_tls(self.tls)
        self.ql.log.debug(
            f"Restored context. tpidr_el0={hex(self.ql.reg.tpidr_el0)}"
        )
...
class QlLinuxThreadManagement:
    ...
    def _prepare_lib_patch(self):
        if self.ql.loader.elf_entry != self.ql.loader.entry_point:
            ...
            if self.ql.archtype == QL_ARCH.ARM and entry_address & 1 == 1:
                entry_address -= 1
            self.main_thread = self.ql.os.thread_class.spawn(
                self.ql, self.ql.loader.entry_point, entry_address
            )
            ...
            if self.ql.reg.arch_pc != entry_address:
                self.ql.log.error(
                    f"{self.cur_thread} Expect {hex(self.ql.loader.elf_entry)} but get {hex(self.ql.reg.arch_pc)} when running loader."
                )
    ...
    def run(self):
        ...
        if previous_thread is None:
            self.main_thread = self.ql.os.thread_class.spawn(
                self.ql, self.ql.loader.elf_entry, self.ql.os.exit_point
            )
        else:
