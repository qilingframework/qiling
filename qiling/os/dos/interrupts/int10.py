...
def __leaf_00(ql: Qiling):
    ...
    if not curses.has_colors():
        ql.log.warning(
            f"your terminal does not support colors, content might not be displayed correctly"
        )
    ...
    if curses.has_colors():
        for fg in range(16):
            for bg in range(16):
                ...
                curses.init_pair(
                    color_pair_index, COLORS_MAPPING[fg], COLORS_MAPPING[bg]
                )
...
def __leaf_06(ql: Qiling):
    ...
    if ch != 0 or cl != 0 or dh != y - 1 or dl != x - 1:
        ql.log.warning(
            f"Partial scroll is unsupported. Will scroll the whole page."
        )
        ql.log.warning(
