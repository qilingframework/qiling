...
class MachHostServer:
    ...
    def host_info(self, in_header, in_content):
        ...
        if flavor == HOST_BASIC_INFO:
            ...
            out_msg.content += pack(
                "<L", host_info_outCnt
            )  # host info output count
            ...
            out_msg.content += pack(
                "<L", 0x50
            )  # kernel_priority = MINPRI_KERNEL;
            out_msg.content += pack(
                "<L", 0x50
            )  # system_priority = MINPRI_KERNEL;
            out_msg.content += pack(
                "<L", 0x40
            )  # server_priority = MINPRI_RESERVED;
            out_msg.content += pack(
                "<L", 0x1F
            )  # user_priority = BASEPRI_DEFAULT;
            out_msg.content += pack("<L", 0x0)  # depress_priority = DEPRESSPRI;
            out_msg.content += pack("<L", 0x0)  # idle_priority = IDLEPRI;
            out_msg.content += pack(
                "<L", 0x0
            )  # minimum_priority = MINPRI_USER;
            out_msg.content += pack(
                "<L", 0x4F
            )  # maximum_priority = MAXPRI_RESERVED
        else:
    ...
    def host_get_clock_service(self, in_header, in_content):
        ...
        out_msg.trailer = b""
        out_msg.trailer += pack(
            "<L", self.ql.os.macho_port_manager.clock_port.name
        )  # clock port name
...
class MachTaskServer:
    ...
    def semaphore_create(self, in_header, in_content):
        ...
        out_msg.trailer = b""
        out_msg.trailer += pack(
            "<L", self.ql.os.macho_port_manager.semaphore_port.name
        )  # clock port name
        out_msg.trailer += pack("<L", 0x0)  # pad1
        out_msg.trailer += pack("<H", 0x0)  # pad2
        out_msg.trailer += pack("<B", 0x11)  # disposition
    ...
    def get_special_port(self, in_header, in_content):
        ...
        out_msg.trailer = b""
        out_msg.trailer += pack(
            "<L", self.ql.os.macho_port_manager.special_port.name
        )  # special port name
        out_msg.trailer += pack("<L", 0x0)  # pad1
        out_msg.trailer += pack("<H", 0x0)  # pad2
        out_msg.trailer += pack("<B", 0x11)  # disposition
