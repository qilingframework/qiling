...
class SharedFileMappingNp:
    ...
    def read_mapping(self, addr):
        ...
        self.ql.dprint(D_PROT, "[ShareFileMapping]: addr: 0x{:X}, size: 0x{:X}, fileOffset:0x{:X}, maxProt: {}, initProt: {}".format(
...
class FileSystem():
    ...
    def get_common_attr(self, path, cmn_flags):
        ...
        if cmn_flags & ATTR_CMN_NAME != 0:
            ...
            self.ql.dprint(D_PROT, "FileName :{}, len:{}".format(filename, filename_len))

        if cmn_flags & ATTR_CMN_DEVID != 0:
            attr += pack("<L", file_stat.st_dev)
            self.ql.dprint(D_PROT, "DevID: {}".format(file_stat.st_dev))
        ...
        if cmn_flags & ATTR_CMN_OBJTYPE != 0:
            if os.path.isdir(path):
                ...
                attr += pack("<L", VLINK)
                self.ql.dprint(D_PROT, "ObjType: LINK")
            else:
                attr += pack("<L", VREG)
                self.ql.dprint(D_PROT, "ObjType: REG")
            
        if cmn_flags & ATTR_CMN_OBJID != 0:
            attr += pack("<Q", file_stat.st_ino)
            self.ql.dprint(D_PROT, "VnodeID :{}".format(file_stat.st_ino))
        ...
        self.ql.dprint(D_PROT, "Attr : {}".format(attr))
