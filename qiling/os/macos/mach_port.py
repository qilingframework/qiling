...
class MachMsgHeader:
    ...
    def read_header_from_mem(self, addr):
        ...
        self.msgh_remote_port = unpack("<L", self.ql.mem.read(addr + 0x8, 0x4))[
            0
        ]
        self.msgh_local_port = unpack("<L", self.ql.mem.read(addr + 0xC, 0x4))[
            0
        ]
        self.msgh_voucher_port = unpack(
            "<L", self.ql.mem.read(addr + 0x10, 0x4)
        )[0]
...
class MachMsg:
    ...
    def read_msg_from_mem(self, addr, size):
        ...
        self.content = self.read_msg_content(
            addr + self.header.header_size, size - self.header.header_size
        )
    ...
    def write_msg_to_mem(self, addr):
        ...
        self.ql.mem.write(
            addr + 0x10, pack("<L", self.header.msgh_voucher_port)
        )
...
class MachPortManager:
    ...
    def deal_with_msg(self, msg, addr):
        ...
        if msg.header.msgh_id == 200:
            # host info
            out_msg = self.ql.os.macho_host_server.host_info(
                msg.header, msg.content
            )
            out_msg.write_msg_to_mem(addr)
        elif msg.header.msgh_id == 206:
            out_msg = self.ql.os.macho_host_server.host_get_clock_service(
                msg.header, msg.content
            )
            out_msg.write_msg_to_mem(addr)
        elif msg.header.msgh_id == 3418:
            out_msg = self.ql.os.macho_task_server.semaphore_create(
                msg.header, msg.content
            )
            out_msg.write_msg_to_mem(addr)
        elif msg.header.msgh_id == 3409:
            out_msg = self.ql.os.macho_task_server.get_special_port(
                msg.header, msg.content
            )
            out_msg.write_msg_to_mem(addr)
        else:
            self.ql.log.info(
                "Error Mach Msgid {} can not handled".format(msg.header.msgh_id)
            )
            raise Exception("Mach Msgid Not Found")

        self.ql.log.debug(
            "Reply-> Header: {}, Content: {}".format(
                out_msg.header, out_msg.content
            )
        )
