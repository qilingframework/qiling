...
@macos_kernel_api(
    passthru=True, params={"s1": STRING, "s2": STRING, "n": SIZE_T}
)
...
@macos_kernel_api(
    passthru=True,
    params={"oidp": POINTER, "arg1": POINTER, "arg2": INT, "req": POINTER},
)
...
def hook___ZN12IOUserClient12initWithTaskEP4taskPvjP12OSDictionary(
    ql, address, params
):
    return 1
...
@macos_kernel_api(params={"grp": POINTER, "grp_name": STRING, "attr": POINTER})
...
def lck_mtx_init(ql, lck, grp, attr):
    ...
    if attr != 0:
        ...
        lck_attr_addr = ql.kernel_extrn_symbols_detail[b"LockDefaultLckAttr"][
            "n_value"
        ]
...
@macos_kernel_api(params={"lck": POINTER, "grp": POINTER, "attr": POINTER})
...
@macos_kernel_api(params={"lock": POINTER})
...
@macos_kernel_api(
    params={"oldValue": POINTER, "newValue": POINTER, "address": POINTER}
)
...
@macos_kernel_api(params={"lock": POINTER, "type": UINT})
...
@macos_kernel_api(params={"buf": POINTER, "size": UINT})
...
@macos_kernel_api(params={"cred": POINTER, "uid": INT, "gid": INT})
...
@macos_kernel_api(params={"oidp": POINTER})
def hook__sysctl_register_oid(ql, address, params):
    ...
    if oidp.oid_handler.value != 0:
        ...
        ql.os.ev_manager.register(
            oidp.oid_handler.value,
            true_name,
            MacOSEventType.EV_SYSCTL,
            ev_obj=oidp,
            idx=0,
        )
    return
...
def hook__sysctl_root(ql, address, params):
    if params["string_is_canonical"] == "True":
        ev_name = ql.mem.read(
            params["namestring"], params["namestringlen"]
        ).decode()
        ev_type = MacOSEventType.EV_SYSCTL
        event = ql.os.ev_manager.get_event_by_name_and_type(ev_name, ev_type)
        if event is not None:
            obj = event.event
            ql.os.ev_manager.emit(
                ev_name,
                ev_type,
                [obj.oid_arg1.value, obj.oid_arg2, params["req"]],
            )
        else:
...
def hook__ctl_register(ql, address, params):
    ...
    if userctl.ctl_connect.value != 0:
        flag |= 1 << 0
        ql.os.ev_manager.register(
            userctl.ctl_connect.value,
            ctl_name,
            MacOSEventType.EV_CTL_CONNECT,
            ev_obj=userctl,
            idx=0,
        )
    if userctl.ctl_disconnect.value != 0:
        flag |= 1 << 1
        ql.os.ev_manager.register(
            userctl.ctl_disconnect.value,
            ctl_name,
            MacOSEventType.EV_CTL_DISCONNECT,
            ev_obj=userctl,
            idx=0,
        )
    if userctl.ctl_send.value != 0:
        flag |= 1 << 2
        ql.os.ev_manager.register(
            userctl.ctl_send.value,
            ctl_name,
            MacOSEventType.EV_CTL_SEND,
            ev_obj=userctl,
            idx=0,
        )
    ...
    if userctl.ctl_rcvd.value != 0:
        flag |= 1 << 5
        ql.os.ev_manager.register(
            userctl.ctl_rcvd.value,
            ctl_name,
            MacOSEventType.EV_CTL_RCVD_FUNC,
            ev_obj=userctl,
            idx=0,
        )
    if userctl.ctl_send_list.value != 0:
        flag |= 1 << 6
        ql.os.ev_manager.register(
            userctl.ctl_send_list.value,
            ctl_name,
            MacOSEventType.EV_CTL_SEND_LIST_FUNC,
            ev_obj=userctl,
            idx=0,
        )
    if userctl.ctl_bind.value != 0:
        flag |= 1 << 7
        ql.os.ev_manager.register(
            userctl.ctl_bind.value,
            ctl_name,
            MacOSEventType.EV_CTL_BIND_FUNC,
            ev_obj=userctl,
            idx=0,
        )

    ql.log.debug(
        "New ctl callbacks has been registered: %s (%d/%d/%d/%d/%d/%d/%d/%d)"
        % (
            ctl_name,
            (flag & (1 << 0)) != 0,
            (flag & (1 << 1)) != 0,
...
@macos_kernel_api(params={"ctlref": POINTER})
...
@macos_kernel_api(
    params={"filter": POINTER, "domain": INT, "type": INT, "protocol": INT}
)
def hook__sflt_register(ql, address, params):
    ...
    if sf.sf_unregistered != 0:
        flag |= 1 << 0
        ql.os.ev_manager.register(
            sf.sf_unregistered.value,
            true_name,
            MacOSEventType.EV_SFLT_UNREGISTERED,
    ...
    if sf.sf_notify != 0:
        ...
        ql.os.ev_manager.register(
            sf.sf_notify.value,
            true_name,
            MacOSEventType.EV_SFLT_NOTIFY_FLUSH_READ,
            protocol=params["protocol"],
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value,
            true_name,
            MacOSEventType.EV_SFLT_NOTIFY_SHUTDOWN,
            protocol=params["protocol"],
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value,
            true_name,
            MacOSEventType.EV_SFLT_NOTIFY_CANTRECVMORE,
            protocol=params["protocol"],
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value,
            true_name,
            MacOSEventType.EV_SFLT_NOTIFY_CANTSENDMORE,
            protocol=params["protocol"],
        )
        ql.os.ev_manager.register(
            sf.sf_notify.value,
    ...
    if sf.sf_getpeername != 0:
        flag |= 1 << 4
        ql.os.ev_manager.register(
            sf.sf_getpeername.value,
            true_name,
            MacOSEventType.EV_SFLT_GETPEERNAME,
            protocol=params["protocol"],
        )
    if sf.sf_getsockname != 0:
        flag |= 1 << 5
        ql.os.ev_manager.register(
            sf.sf_getsockname.value,
            true_name,
            MacOSEventType.EV_SFLT_GETSOCKNAME,
            protocol=params["protocol"],
        )
    if sf.sf_data_in != 0:
        flag |= 1 << 6
    ...
    if sf.sf_ioctl != 0:
        ...
        ql.os.ev_manager.register(
            sf.sf_ioctl.value,
            true_name,
            MacOSEventType.EV_SFLT_IOCTL,
            protocol=params["protocol"],
        )

    ql.log.debug(
        "New sflt callbacks has been registered: %s (%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d/%d)"
        % (
            true_name,
            (flag & (1 << 0)) != 0,
            (flag & (1 << 1)) != 0,
...
def hook__sflt_unregister(ql, address, params):
    ...
    for found in evs:
        ql.os.ev_manager.emit(
            found.name, MacOSEventType.EV_SFLT_UNREGISTERED, [params["handle"]]
        )
        break
...
def hook__mac_policy_register(ql, address, params):
    ...
    for i in range(0, NUM_EVENT_MAC_POLICY):
        ...
        if func != 0:
            ql.os.ev_manager.register(
                ...
                true_name,
                MACPolicy_EventType[
                    MACPolicy_EventType(i + base_event_MAC).name
                ],
            )
            ql.log.debug(
                "New mac policy callback has been registered: Name: %s --- Type: %s --- Addr: 0x%x "
                % (
                    true_name,
                    MACPolicy_EventType(i + base_event_MAC).name,
                    func,
    ...
    ql.os.ev_manager.emit_by_type(
        MACPolicy_EventType.EV_MAC_mpo_policy_init, [params["mpc"]]
    )
    ql.os.ev_manager.emit_by_type(
        MACPolicy_EventType.EV_MAC_mpo_policy_initbsd, [params["mpc"]]
    )

    return 0


@macos_kernel_api(params={"handle": POINTER})
def hook__mac_policy_unregister(ql, address, params):
    ...
    ql.os.ev_manager.emit_by_type(
        MACPolicy_EventType.EV_MAC_mpo_policy_destroy, [mpc.base]
    )
...
def hook__kauth_listen_scope(ql, address, params):
    ev_name = (
        ql.mem.string(params["_identifier"])
        .replace("com.", "")
        .replace("apple.", "")
        .upper()
        .replace(".", "_")
    ...
    ql.os.ev_manager.register(
        params["_callback"], ev_name.encode(), MacOSEventType["EV_" + ev_name]
    )
...
def hook__ipf_addv4(ql, address, params):
    ...
    if ip_filter_ipv4.ipf_input.value > 0:
        flag |= 1 << 0
        ql.os.ev_manager.register(
            ip_filter_ipv4.ipf_input.value, name, MacOSEventType.EV_IPF_INPUT
        )

    if ip_filter_ipv4.ipf_output.value > 0:
        flag |= 1 << 1
        ql.os.ev_manager.register(
            ip_filter_ipv4.ipf_output.value, name, MacOSEventType.EV_IPF_OUTPUT
        )

    if ip_filter_ipv4.ipf_detach.value > 0:
        flag |= 1 << 2
        ql.os.ev_manager.register(
            ip_filter_ipv4.ipf_detach.value, name, MacOSEventType.EV_IPF_DETACH
        )

    ql.log.debug(
        "New ipf callbacks has been registered: %s (%d/%d/%d)"
        % (
            name,
            (flag & (1 << 0)) != 0,
            (flag & (1 << 1)) != 0,
...
def hook__getattrlistbulk(ql, address, params):
    getattrlistbulk_args = getattrlistbulk_args_t(
        ql, params["getattrlistbulk_args"]
    ).loadFromMem()
    dirfd = ql.os.ev_manager.map_fd[getattrlistbulk_args.dirfd]

    vfs_attr_pack = ql.loader.kernel_extrn_symbols_detail[b"_vfs_attr_pack"][
        "n_value"
    ]
    ...
    uio.uio_iovs = iovecs_t(
        kiovp=POINTER64(uiovp_addr), uiovp=POINTER64(uiovp_addr)
    )
    uio.uio_iovcnt = 1
    uio.uio_offset = 0
    ...
    for path in dirfd.iterdir():
        ...
        vap.va_create_time = timespec_t(
            tv_sec=int(info.st_ctime), tv_nsec=info.st_ctime_ns % 1000000
        )
        vap.va_access_time = timespec_t(
            tv_sec=int(info.st_atime), tv_nsec=info.st_atime_ns % 1000000
        )
        vap.va_modify_time = timespec_t(
            tv_sec=int(info.st_mtime), tv_nsec=info.st_mtime_ns % 1000000
        )
        ...
        code = gen_stub_code(
            ...
            [
                0,
                uio.base,
                getattrlistbulk_args.alist,
                getattrlistbulk_args.options,
                vap.base,
                0,
                params["p"],
            ],
        ...
        print(
            "Trampoline created at 0x%x for %s (0x%x) and 0x%x"
            % (code, truename, vap.va_name.value, vap.base)
        )
        ql.stack_push(code)
