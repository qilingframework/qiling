...
def ql_syscall_fgetattrlist(
    ql, fd, alist, attributeBuffer, bufferSize, options, *args, **kw
):
    ql.log.debug(
        "fgetattrlist(fd: 0x%x, alist: 0x%x, attributeBuffer: 0x%x, bufferSize: 0x%x, options: 0x%x)"
    ...
    ql.log.debug(
        "bitmapcount: 0x%x, reserved: 0x%x, commonattr: 0x%x, volattr: 0x%x, dirattr: 0x%x, fileattr: 0x%x, forkattr: 0x%x\n"
        % (
    ...
    if attrlist["commonattr"] != 0:
        commonattr = ql.os.macho_fs.get_common_attr(
            ql.path, attrlist["commonattr"]
        )
        if not commonattr:
...
# 0xa
def ql_syscall_kernelrpc_mach_vm_allocate_trap(
    ql, port, addr, size, flags, *args, **kw
):
    ql.log.debug(
        "[mach] mach vm allocate trap(port: 0x%x, addr: 0x%x, size: 0x%x, flags: 0x%x"
        % (port, addr, size, flags)
    )
...
# 0xc
def ql_syscall_kernelrpc_mach_vm_deallocate_trap(
    ql, target, address, size, *args, **kw
):
...
# 0xf
def ql_syscall_kernelrpc_mach_vm_map_trap(
    ql, target, address, size, mask, flags, cur_protection
):
    ql.log.debug(
        "[mach] mach vm map trap(target: 0x%x, address: 0x%x, size: 0x%x, mask: 0x%x, flag: 0x%x, cur_protect: 0x%x)"
        % (target, address, size, mask, flags, cur_protection)
    )

    if ql.os.macho_vmmap_end & mask > 0:
        ql.os.macho_vmmap_end = ql.os.macho_vmmap_end - (
            ql.os.macho_vmmap_end & mask
        )
...
# 0x13
def ql_syscall_kernelrpc_mach_port_mod_refs_trap(
    ql, target, name, right, delta, *args, **kw
):
    ql.log.debug(
        "[mach] mach port mod refs trap(target: 0x%x, name: 0x%x, right: 0x%x, delta: 0x%x)"
...
# 0x18
def ql_syscall_kernelrpc_mach_port_construct_trap(
    ql, target, options, context, name, *args, **kw
):
    ql.log.debug(
        "[mach] mach port construct trap(target: 0x%x, options: 0x%x, context: 0x%x, name: 0x%x)"
...
def ql_syscall_mach_reply_port(ql, *args, **kw):
    ql.log.debug(
        "[mach] mach reply port , ret: %s" % (ql.os.macho_mach_port.name)
    )
...
def ql_syscall_host_self_trap(ql, *args, **kw):
    port_manager = ql.os.macho_port_manager
    ql.log.debug(
        "[mach] host_self_trap, ret: %s"
        % (ql.os.macho_port_manager.host_port.name)
    )
...
def ql_syscall_mach_msg_trap(ql, args, opt, ssize, rsize, rname, timeout):
    ql.log.debug(
        "[mach] mach_msg_trap(args: 0x%x opt: 0x%x, ssize: 0x%x, rsize: 0x%x, rname: 0x%x, timeout: %d)"
        % (args, opt, ssize, rsize, rname, timeout)
    )
    mach_msg = MachMsg(ql)
    mach_msg.read_msg_from_mem(args, ssize)
    ql.log.debug(
        "Recv-> Header: %s, Content: %s" % (mach_msg.header, mach_msg.content)
    )
...
def ql_syscall_sigprocmask(ql, how, mask, omask, *args, **kw):
    ql.log.debug(
        "sigprocmask(how: 0x%x, mask: 0x%x, omask: 0x%x)" % (how, mask, omask)
    )
...
def ql_syscall_fcntl64_macos(ql, fcntl_fd, fcntl_cmd, fcntl_arg, *args, **kw):
    ...
    ql.log.debug(
        "fcntl64(fd: %d, cmd: %d, arg: 0x%x) = %d"
        % (fcntl_fd, fcntl_cmd, fcntl_arg, regreturn)
    )
    return regreturn
...
def ql_syscall_pread(ql, fd, buf, nbyte, offset, *args, **kw):
    ql.log.debug(
        "pread(fd: 0x%x, buf: 0x%x, nbyte: 0x%x, offset: 0x%x)"
        % (fd, buf, nbyte, offset)
    )
...
def ql_syscall_csops(ql, pid, ops, useraddr, usersize, *args, **kw):
    ...
    ql.log.debug(
        "csops(pid: %d, ops: 0x%x, useraddr: 0x%x, usersize: 0x%x) flag: 0x%x"
...
# 0xdc
def ql_syscall_getattrlist(
    ql, path, alist, attributeBuffer, bufferSize, options, *args, **kw
):
    ql.log.debug(
        "getattrlist(path: 0x%x, alist: 0x%x, attributeBuffer: 0x%x, bufferSize: 0x%x, options: 0x%x)"
...
def ql_syscall_sysctl(ql, name, namelen, old, oldlenp, new_arg, newlen):
    ql.log.debug(
        "sysctl(name: 0x%x, namelen: 0x%x, old: 0x%x, oldlenp: 0x%x, new: 0x%x, newlen: 0x%x)"
...
def ql_syscall_sysctlbyname(ql, name, namelen, old, oldlenp, new_arg, newlen):
    ql.log.debug(
        "sysctlbyname(name: 0x%x, namelen: 0x%x, old: 0x%x, oldlenp: 0x%x, new: 0x%x, newlen: 0x%x)"
        % (name, namelen, old, oldlenp, new_arg, newlen)
    )
    return KERN_SUCCESS
...
def ql_syscall_shared_region_check_np(ql, p, uap, retvalp, *args, **kw):
    ql.log.debug(
        "shared_region_check_np(p: 0x%x, uap: 0x%x, retvalp: 0x%x) = 0x%x"
        % (p, uap, retvalp, EINVAL)
    )
    return EINVAL
...
def ql_syscall_proc_info(ql, callnum, pid, flavor, arg, buff, buffer_size):
    ...
    ql.log.debug(
        "proc_info(callnum: 0x%x, pid: %d, flavor:0x%x, arg: 0x%x, buffer: 0x%x, buffersize: 0x%x, retval: 0x%x)"
...
# 0x16e
def ql_syscall_bsdthread_register(
    ql,
    threadstart,
    wqthread,
    flags,
...
# 0x18d
def ql_syscall_write_nocancel(
    ql, write_fd, write_buf, write_count, *args, **kw
):
    regreturn = 0
    buf = None
    ...
    try:
        ...
        if hasattr(ql.os.fd[write_fd], "write"):
            ...
            ql.log.debug(
                "write(%d,%x,%i) failed due to write_fd"
...
# 0x1b6
def ql_syscall_shared_region_map_and_slide_np(
    ql, fd, count, mappings_addr, slide, slide_start, slide_size
):
    ql.log.debug(
        "shared_region_map_and_slide_np(fd: %d, count: 0x%x, mappings: 0x%x, slide: 0x%x, slide_start: 0x%x, slide_size: 0x%x)"
...
def ql_syscall_csrctl(ql, op, useraddr, usersize, *args, **kw):
    ql.log.debug(
        "csrctl(op: 0x%x, useraddr :0x%x, usersize: 0x%x)"
...
def ql_syscall_terminate_with_payload(
    ql, pid, reason_namespace, reason_code, payload, payload_size, reason_string
):
    ql.log.debug(
        "terminate_with_payload(pid: %d, reason_namespace: 0x%x, reason_code: 0x%x, payload: 0x%x \
            payload_size: 0x%x, reason_string: 0x%x)"
        % (
            pid,
            reason_namespace,
            reason_code,
...
# 0x209
def ql_syscall_abort_with_payload(
    ql,
    reason_namespace,
    reason_code,
    payload,
    ...
    ql.log.debug(
        "abort_with_payload(reason_namespace: 0x%x, reason_code: 0x%x, payload: 0x%x, payload_size: 0x%x, reason_string: 0x%x,\
            reason_flags: 0x%x)"
        % (
            reason_namespace,
            reason_code,
...
def ql_syscall_thread_fast_set_cthread_self64(ql, u_info_addr, *args, **kw):
    ql.log.debug(
        "[mdep] thread fast set cthread self64(tsd_base:0x%x)" % (u_info_addr)
    )
