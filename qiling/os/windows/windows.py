...
class QlOsWindows(QlOs):
    def __init__(self, ql: Qiling):
        ...
        self.automatize_input = self.profile.getboolean(
            "MISC", "automatize_input"
        )
        self.username = self.profile["USER"]["username"]
        self.windir = (
            self.profile["PATH"]["systemdrive"] + self.profile["PATH"]["windir"]
        )
        self.userprofile = (
            self.profile["PATH"]["systemdrive"]
            + "Users\\"
            ...
            if api_func:
                ...
                if ql.debug_stop:
                    raise QlErrorSyscallNotFound(
                        "Windows API implementation not found"
                    )
    ...
    def post_report(self):
        ...
        for key, values in self.utils.appeared_strings.items():
            self.ql.log.debug(
                f'{key}: {" ".join(str(word) for word in values)}'
            )
        ...
        try:
            if self.ql.code:
                self.ql.emu_start(
                    ...
                    (self.ql.loader.entry_point + len(self.ql.code)),
                    self.ql.timeout,
                    self.ql.count,
                )
            else:
