...
def _GetModuleHandle(self, address, params):
    ...
    if lpModuleName == 0:
        ret = self.LoaderPE.PE_IMAGE_BASE
    else:
        lpModuleName = lpModuleName.lower()
        if not is_file_library(lpModuleName):
            lpModuleName += ".dll"
        if lpModuleName in self.LoaderPE.dlls:
            ret = self.LoaderPE.dlls[lpModuleName]
        else:
            self.ql.dprint(0, "[!] Library %s not imported" % lpModuleName)
            ...
            if is_file_library(lpModuleName) and os.path.exists(path):
                ret = self.LoaderPE.load_dll(lpModuleName.encode())
            else:
                self.ql.dprint(0, "[!] Library %s not found" % lpModuleName)
                ret = 0
    return ret
...
def hook_GetModuleFileNameA(self, address, params):
    ...
    if hModule == 0 or hModule == self.LoaderPE.PE_IMAGE_BASE:
        filename = self.LoaderPE.filepath
        filename_len = len(filename)
        if filename_len > nSize - 1:
            filename = self.LoaderPE.filepath[:nSize - 1]
            ret = nSize
        else:
            ret = filename_len
        self.ql.mem.write(lpFilename, filename + b"\x00")
    else:
        self.ql.dprint(0, "hModule %x" % hModule)
...
def hook_GetModuleFileNameW(self, address, params):
    ...
    # Pretty much 0 and PE_IMAGE_BASE value should do the same operations
    if hModule == 0 or hModule == self.LoaderPE.PE_IMAGE_BASE:
        filename = self.LoaderPE.filepath.decode('ascii').encode('utf-16le')
        filename_len = len(filename)
        if filename_len > nSize - 1:
            filename = self.LoaderPE.filepath[:nSize - 1]
            ret = nSize
...
def hook_GetProcAddress(self, address, params):
    ...
    try:
        dll_name = [key for key, value in self.LoaderPE.dlls.items() if value == params['hModule']][0]
    except IndexError as ie:
        self.ql.nprint('[!] Failed to import function "%s" with handle 0x%X' % (lpProcName, params['hModule']))
        return 0

    if lpProcName in self.LoaderPE.import_address_table[dll_name]:
        return self.LoaderPE.import_address_table[dll_name][lpProcName]
...
def hook_LoadLibraryA(self, address, params):
    ...
    dll_base = self.LoaderPE.load_dll(lpLibFileName.encode())
    return dll_base
...
def hook_LoadLibraryW(self, address, params):
    ...
    dll_base = self.LoaderPE.load_dll(lpLibFileName)
    return dll_base
