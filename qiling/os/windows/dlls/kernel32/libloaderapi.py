...
def _GetModuleHandle(self, address, params):
    ...
    if lpModuleName == 0:
        ret = self.PE.PE_IMAGE_BASE
    else:
        lpModuleName = lpModuleName.lower()
        if not is_file_library(lpModuleName):
            lpModuleName += ".dll"
        if lpModuleName in self.PE.dlls:
            ret = self.PE.dlls[lpModuleName]
        else:
            ...
            if is_file_library(lpModuleName) and os.path.exists(path):
                ret = self.PE.load_dll(lpModuleName.encode())
            else:
...
def hook_GetModuleFileNameA(self, address, params):
    ...
    if hModule == 0 or hModule == self.PE.PE_IMAGE_BASE:
        filename = self.PE.filepath
        filename_len = len(filename)
        if filename_len > nSize - 1:
            filename = self.PE.filepath[:nSize - 1]
            ret = nSize
        else:
            ret = filename_len
...
def hook_GetModuleFileNameW(self, address, params):
    ...
    # Pretty much 0 and PE_IMAGE_BASE value should do the same operations
    if hModule == 0 or hModule == self.PE.PE_IMAGE_BASE:
        filename = self.PE.filepath.decode('ascii').encode('utf-16le')
        filename_len = len(filename)
        if filename_len > nSize - 1:
            filename = self.PE.filepath[:nSize - 1]
            ret = nSize
...
def hook_GetProcAddress(self, address, params):
    ...
    try:
        dll_name = [key for key, value in self.PE.dlls.items() if value == params['hModule']][0]
    except IndexError as ie:
        self.ql.nprint('[!] Failed to import function "%s" with handle 0x%X' % (lpProcName, params['hModule']))
        return 0

    if lpProcName in self.PE.import_address_table[dll_name]:
        return self.PE.import_address_table[dll_name][lpProcName]
...
def hook_LoadLibraryA(self, address, params):
    ...
    dll_base = self.PE.load_dll(lpLibFileName.encode())
    return dll_base
...
def hook_LoadLibraryW(self, address, params):
    ...
    dll_base = self.PE.load_dll(lpLibFileName)
    return dll_base
