...
def hook_TlsFree(ql, address, params):
    ...
    if idx not in ql.thread_manager.current_thread.tls:
        ql.comm_os.last_error = 0x57  # (ERROR_INVALID_PARAMETER)
        return 0
    else:
...
def hook_TerminateProcess(ql, address, params):
    ...
    if process == 0x0 or process == ql.comm_os.DEFAULT_IMAGE_BASE:
        ql.uc.emu_stop()
        ql.comm_os.PE_RUN = False
    ret = 1
    return ret
...
def hook_OpenProcess(ql, address, params):
    ...
    if proc == 0:
        ql.comm_os.last_error  = ERROR_INVALID_PARAMETER
        return 0
    return 0xD10C
