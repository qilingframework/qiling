...
def hook_GetFileType(ql, address, params):
    hFile = params["hFile"]
    if (
        hFile == STD_INPUT_HANDLE
        or hFile == STD_OUTPUT_HANDLE
        or hFile == STD_ERROR_HANDLE
    ):
        ret = FILE_TYPE_CHAR
    else:
...
# );
@winsdkapi(
    cc=STDCALL, dllname=dllname, replace_params_type={"LPCSTR": "POINTER"}
)
...
@winsdkapi(
    cc=STDCALL,
    dllname=dllname,
    replace_params={"nBufferLength": DWORD, "lpBuffer": POINTER},
)
...
def hook_GetVolumeInformationW(ql, address, params):
    ...
    if root != 0:
        ...
        if pt_volume_name != 0:
            # TODO implement
            volume_name = (ql.os.profile["VOLUME"]["name"] + "\x00").encode(
                "utf-16le"
            )
        ...
        if lpMaximumComponentLength != 0:
            ql.mem.write(
                lpMaximumComponentLength, (255).to_bytes(2, byteorder="little")
            )
        pt_serial_number = params["lpVolumeSerialNumber"]
        if pt_serial_number != 0:
            # TODO maybe has to be int
            serial_number = (
                ql.os.profile["VOLUME"]["serial_number"] + "\x00"
            ).encode("utf-16le")
        ...
        if pt_system_type != 0:
            system_type = (ql.os.profile["VOLUME"]["type"] + "\x00").encode(
                "utf-16le"
            )
...
def hook_GetDiskFreeSpaceW(ql, address, params):
    ...
    if path == ql.os.profile["PATH"]["systemdrive"]:
        ...
        sectors = ql.os.profile.getint(
            "VOLUME", "sectors_per_cluster"
        ).to_bytes(4, byteorder="little")
        bytes = ql.os.profile.getint("VOLUME", "bytes_per_sector").to_bytes(
            4, byteorder="little"
        )
        free_clust = ql.os.profile.getint(
            "VOLUME", "number_of_free_clusters"
        ).to_bytes(4, byteorder="little")
        total_clust = ql.os.profile.getint(
            "VOLUME", "number_of_clusters"
        ).to_bytes(4, byteorder="little")
...
# );
@winsdkapi(
    cc=STDCALL, dllname=dllname, replace_params_type={"LPDWORD": "DWORD"}
)
...
# );
@winsdkapi(
    cc=STDCALL, dllname=dllname, replace_params={"lpBaseAddress": POINTER}
)
...
def hook_CopyFileA(ql, address, params):
    lpExistingFileName = ql.os.path.transform_to_real_path(
        params["lpExistingFileName"]
    )
...
@winsdkapi(
    cc=STDCALL,
    dllname=dllname,
    replace_params={"lpFileName": STRING, "dwFileAttributes": DWORD},
)
