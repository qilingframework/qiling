...
def hook_GetSystemInfo(self, address, params):
    ...
    system_info = {"dummy": 0x0.to_bytes(length=2 * 2 + 4, byteorder='little'),
                   ...
                   "lpMinimumApplicationAddress": self.PE.PE_IMAGE_BASE.to_bytes(length=self.ql.pointersize, byteorder='little'),
                   "lpMaximumApplicationAddress": (self.PE.DLL_BASE_ADDR + self.PE.DLL_SIZE).to_bytes(length=self.ql.pointersize,
                                                                                            byteorder='little'),
...
def hook_GetWindowsDirectoryW(self, address, params):
    dst = params["lpBuffer"]
    value = (self.ql.config["PATHS"]["windir"] + "\x00").encode("utf-16le")
