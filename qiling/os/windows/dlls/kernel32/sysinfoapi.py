...
def hook_GetSystemInfo(ql, address, params):
    ...
    system_info = {"dummy": 0x0.to_bytes(length=2 * 2 + 4, byteorder='little'),
                   ...
                   "lpMinimumApplicationAddress": ql.comm_os.PE_IMAGE_BASE.to_bytes(length=ql.pointersize, byteorder='little'),
                   "lpMaximumApplicationAddress": (ql.comm_os.DLL_BASE_ADDR + ql.comm_os.DLL_SIZE).to_bytes(length=ql.pointersize,
                                                                                            byteorder='little'),
