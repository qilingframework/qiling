...
def hook_GetSystemInfo(self, address, params):
    ...
    system_info = {"dummy": 0x0.to_bytes(length=2 * 2 + 4, byteorder='little'),
                   ...
                   "lpMinimumApplicationAddress": self.loader.PE_IMAGE_BASE.to_bytes(length=self.ql.pointersize, byteorder='little'),
                   "lpMaximumApplicationAddress": (self.loader.DLL_BASE_ADDR + self.loader.DLL_SIZE).to_bytes(length=self.ql.pointersize,
                                                                                            byteorder='little'),
