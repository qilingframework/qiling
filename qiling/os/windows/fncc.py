            ...
            for fname, fparams in obj.items():
                ...
                for p in fparams:
                    ...
                    if type(ptype) is str:
                        ...
                        # <workaround>
                        _ptype = getattr(const, ptype, None) or reptypedict.get(
                            ptype
                        )
...
def winsdkapi(
    cc: int,
    dllname: str = None,
    replace_params_type: Mapping[str, str] = {},
    replace_params={},
    passthru: bool = False,
):
    """
    ...
    def decorator(func):
        ...
        def wrapper(ql: Qiling, pc: int, api_name: str):
                ...
                if api_name in funcdefs:
                    ...
                    if len(paramlist) == len(replace_params):
                        ...
                        for pname, ptype in params.items():
                            if type(ptype) is str:
                                params[pname] = replacetype(
                                    ptype, replace_params_type
                                )

                                if params[pname] is None:
                                    ql.log.exception(
                                        f'no replacement found for type "{ptype}" ({api_name}, {dllname})'
                                    )

                    # params list indicates that function prototype has no arguments
                    elif (
                        len(paramlist) == 1 and paramlist.get("VOID") == "void"
                    ):
                        params = {}
            ...
            return ql.os.call(
                pc, func, params, onenter, onexit, passthru=passthru
            )
