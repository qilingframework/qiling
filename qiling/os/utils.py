...
class QlOsUtils:
    ...
    def print_function(
        ...
        faddr = (
            f"{address:#0{self.ql.archbit // 4 + 2}x}: "
    ...
    def __common_printf(
        self, format: str, args: MutableSequence, wstring: bool
    ):
    ...
    def va_list(self, format: str, ptr: int) -> MutableSequence[int]:
        ...
        return [
            self.ql.unpack(
                self.ql.mem.read(
                    ptr + i * self.ql.pointersize, self.ql.pointersize
                )
            )
    ...
    def printf(
        self, format: str, args: MutableSequence, wstring: bool = False
    ) -> int:
        out = self.__common_printf(format, args, wstring)
        enc = "utf-8"
    ...
    def update_ellipsis(self, params: MutableMapping, args: Sequence) -> None:
        params.update(
            (f"{QlOsUtils.ELLIPSIS_PREF}{i}", a) for i, a in enumerate(args)
        )
    ...
    def disassembler(self, ql, address, size):
        ...
        ):  # Update disassembler for arm considering thumb swtich.
            self.md = self.ql.create_disassembler()
    ...
    def io_Write(self, in_buffer):
        ...
        if self.ql.archbit == 32:
            buf = self.ql.mem.read(
                self.ql.loader.driver_object.DeviceObject,
        ...
        def build_mdl(buffer_size, data=None):
            ...
            if data:
                written = (
                    data if len(data) <= buffer_size else data[:buffer_size]
                )
        ...
        if self.ql.archtype == QL_ARCH.X8664:
            ...
            # setup IRP structure
            irp = IRP64()
            irp.irpstack = ctypes.cast(
                irpstack_addr, ctypes.POINTER(IO_STACK_LOCATION64)
            )
        else:
            ...
            irp = IRP32()
            irp.irpstack = ctypes.cast(
                irpstack_addr, ctypes.POINTER(IO_STACK_LOCATION32)
            )
        ...
        self.ql.os.fcall.writeParams(
            (
                (POINTER, self.ql.loader.driver_object.DeviceObject),
                (POINTER, irp_addr),
            )
        )
    ...
    def ioctl(self, params):
        ...
        if self.ql.ostype == QL_OS.WINDOWS:
            ...
            if (
                self.ql.loader.driver_object.MajorFunction[
            ...
            self.ql.log.info(
                "IRP is at 0x%x, IO_STACK_LOCATION is at 0x%x"
            ...
            irpstack.Parameters.DeviceIoControl.Type3InputBuffer.value = (
                input_buffer_addr  # used by IOCTL_METHOD_NEITHER
            )
            self.ql.mem.write(irpstack_addr, bytes(irpstack))

            if ctl_method == METHOD_NEITHER:
                irp.UserBuffer.value = (
                    output_buffer_addr  # used by IOCTL_METHOD_NEITHER
                )
            ...
            if io_status.Status.Status >= 0:
                if ctl_method == METHOD_BUFFERED:
                    output_data = self.ql.mem.read(
                        system_buffer_addr, io_status.Information.value
                    )
                if ctl_method in (METHOD_IN_DIRECT, METHOD_OUT_DIRECT):
                    output_data = self.ql.mem.read(
                        mdl.MappedSystemVa.value, io_status.Information.value
                    )
                if ctl_method == METHOD_NEITHER:
                    output_data = self.ql.mem.read(
                        output_buffer_addr, io_status.Information.value
                    )
