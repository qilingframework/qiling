...
def ql_create_disassembler(
    archtype: QL_ARCH, archendian: QL_ENDIAN, reg_cpsr=None
) -> Cs:
    if archtype == QL_ARCH.X86:
        ...
        mode = (
            CS_MODE_THUMB
            if reg_cpsr & __reg_cpsr_v[archendian]
            else CS_MODE_ARM
        )

        md = Cs(
            CS_ARCH_ARM, mode
        )  # FIXME: should be: mode + __cs_endian[archendian]
...
def ql_create_assembler(
    ...
    if archtype == QL_ARCH.X86:
        ...
        ks = Ks(
            KS_ARCH_ARM, mode
        )  # FIXME: should be: mode + __ks_endian[archendian]
