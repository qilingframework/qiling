...
class Qltool_Test(unittest.TestCase):
    def test_qltool_exec_args(self):
        ...
        p = subprocess.Popen(
            create, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
    ...
    def test_qltool_shellcode(self):
        create = [
            ...
            "asm",
            "-f",
            "../examples/shellcodes/lin32_execve.asm",
        ]
        try:
    ...
    def test_qltool_coverage(self):
        ...
        create = [
            ...
            "drcov",
            "--coverage-file",
            "log_test/TcgPlatformSetupPolicy",
        ]
        try:
            subprocess.check_output(create, stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
    ...
    def test_qltool_filter(self):
        ...
        self.assertTrue(
            all(
                filter(
                    lambda x: x.startswith("open") or x.startswith("brk"), lines
                )
            )
        )
