...
import sys, subprocess, string, random
sys.path.append("..")
from qiling import *
...
def test_elf_linux_x86_posix_syscall():
...
def test_elf_linux_mips32_static():
    def random_generator(size=6, chars=string.ascii_uppercase + string.digits):
        return ''.join(random.choice(chars) for x in range(size))
...
def test_elf_linux_mips32el_posix_syscall():
...
def test_elf_linux_execve_x8664():
