...
def hook_mem_invalid(uc, access, address, size, value, user_data):
    ...
    if access == UC_MEM_WRITE:
        info = (
            "invalid WRITE of 0x%x at 0x%X, data size = %u, data value = 0x%x"
            % (address, pc, size, value)
        )
    if access == UC_MEM_READ:
        info = "invalid READ of 0x%x at 0x%X, data size = %u" % (
            address,
            pc,
            size,
        )
...
class TestAndroid(unittest.TestCase):
    def test_edl_arm64(self):
        ...
        replace_function(
            ql, 0x1485C614, devprg_tx_blocking
        )  # Function being used by UART in DP_LOGI

        ql.reg.sp = 0x146B2000  # SP from main
        xml_buffer_addr = (
            0x14684E80  # We extracted that from devprg_get_xml_buffer
        )
