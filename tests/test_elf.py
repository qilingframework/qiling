...
class ELFTest(unittest.TestCase):
    def test_libpatch_elf_linux_x8664(self):
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/patch_test.bin"],
            "../examples/rootfs/x8664_linux",
        )
        ql.patch(
            0x0000000000000575, b"qiling\x00", file_name=b"libpatch_test.so"
        )
        ql.run()
        del ql
    ...
    def test_elf_partial_linux_x8664(self):
        ...
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/sleep_hello"],
            "../examples/rootfs/x8664_linux",
    ...
    def test_elf_x_only_segment(self):
        ...
        ql = Qiling(
            [
                "../examples/rootfs/x8664_linux/bin/sleep_hello_with_x_only_segment"
            ],
            "../examples/rootfs/x8664_linux",
            verbose=QL_VERBOSE.DEBUG,
        )
    ...
    def test_elf_hijackapi_linux_x8664(self):
        ...
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/x8664_puts"],
            "../examples/rootfs/x8664_linux",
            verbose=QL_VERBOSE.DEBUG,
        )
    ...
    def test_elf_linux_x86_posix_syscall(self):
        def test_syscall_read(ql, read_fd, read_buf, read_count, *args):
            ...
            if pathname == "test_syscall_read.txt":
                print(
                    "test => read(%d, %s, %d)" % (read_fd, pathname, read_count)
                )
                target = True

            regreturn = syscall.ql_syscall_read(
                ql, read_fd, read_buf, read_count, *args
            )
            ...
            if target:
                ...
                with open(real_path) as fd:
                    assert (
                        fd.read() == ql.mem.read(read_buf, read_count).decode()
                    )
        ...
        def test_syscall_write(ql, write_fd, write_buf, write_count, *args):
            ...
            regreturn = syscall.ql_syscall_write(
                ql, write_fd, write_buf, write_count, *args
            )
        ...
        def test_syscall_openat(
            ql, openat_fd, openat_path, openat_flags, openat_mode, *args
        ):
            target = False
            ...
            regreturn = syscall.ql_syscall_openat(
                ql, openat_fd, openat_path, openat_flags, openat_mode, *args
            )
        ...
        def test_syscall_truncate(ql, trunc_pathname, trunc_length, *args):
            ...
            regreturn = syscall.ql_syscall_truncate(
                ql, trunc_pathname, trunc_length, *args
            )
        ...
        def test_syscall_ftruncate(ql, ftrunc_fd, ftrunc_length, *args):
            ...
            if pathname == "test_syscall_ftruncate.txt":
                print(
                    "test => ftruncate(%d, 0x%x)" % (ftrunc_fd, ftrunc_length)
                )
                target = True

            regreturn = syscall.ql_syscall_ftruncate(
                ql, ftrunc_fd, ftrunc_length, *args
            )
    ...
    def test_elf_onEnter_mips32el(self):
        ...
        ql = Qiling(
            ["../examples/rootfs/mips32el_linux/bin/mips32el_double_hello"],
    ...
    def test_elf_linux_arm64_posix_syscall(self):
        ...
        ql = Qiling(
            ["../examples/rootfs/arm64_linux/bin/arm64_posix_syscall"],
            "../examples/rootfs/arm64_linux",
            verbose=QL_VERBOSE.DEBUG,
        )
    ...
    def test_elf_linux_mips32el_posix_syscall(self):
        ...
        def test_syscall_open(ql, open_pathname, open_flags, open_mode, *args):
            ...
            regreturn = syscall.ql_syscall_open(
                ql, open_pathname, open_flags, open_mode, *args
            )
    ...
    def test_x86_fake_urandom(self):
        ...
        ql = Qiling(
            ["../examples/rootfs/x86_linux/bin/x86_fetch_urandom"],
            "../examples/rootfs/x86_linux",
            verbose=QL_VERBOSE.DEBUG,
        )
        ql.add_fs_mapper("/dev/urandom", Fake_urandom())
    ...
    def test_x8664_map_urandom(self):
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/x8664_fetch_urandom"],
            "../examples/rootfs/x8664_linux",
            verbose=QL_VERBOSE.DEBUG,
        )
    ...
    def test_x8664_symlink(self):
        ql = Qiling(
            ["../examples/rootfs/x8664_linux_symlink/bin/x8664_hello"],
            "../examples/rootfs/x8664_linux_symlink",
            verbose=QL_VERBOSE.DEBUG,
        )
        ql.run()
        del ql
    ...
    def test_arm_directory_symlink(self):
        ...
        self.assertTrue(
            real_path.endswith(
                "/examples/rootfs/arm_linux/tmp/media/nand/symlink_test/libsymlink_test.so"
            )
        )
        del ql
    ...
    def test_x8664_absolute_path(self):
        ...
        pipe = MyPipe()
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/absolutepath"],
            "../examples/rootfs/x8664_linux",
            verbose=QL_VERBOSE.DEBUG,
            stdout=pipe,
        )
    ...
    def test_x8664_getcwd(self):
        ...
        pipe = MyPipe()
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/testcwd"],
            "../examples/rootfs/x8664_linux",
            verbose=QL_VERBOSE.DEBUG,
            stdout=pipe,
        )
    ...
    def test_elf_linux_x86_return_from_main_stackpointer(self):
        ql = Qiling(
            ["../examples/rootfs/x86_linux/bin/x86_return_main"],
            "../examples/rootfs/x86_linux",
    ...
    def test_elf_linux_x8664_return_from_main_stackpointer(self):
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/x8664_return_main"],
            "../examples/rootfs/x8664_linux",
            stop_on_stackpointer=True,
        )
        ql.run()
    ...
    def test_elf_linux_x8664_getdents(self):
        output = io.BytesIO()
        ql = Qiling(
            ["../examples/rootfs/x8664_linux/bin/x8664_getdents"],
            "../examples/rootfs/x8664_linux",
            verbose=QL_VERBOSE.DEBUG,
            stdout=output,
        )
    ...
    def test_elf_linux_x86_getdents64(self):
        ...
        pipe = MyPipe()
        ql = Qiling(
            ["../examples/rootfs/x86_linux/bin/x86_getdents64"],
            "../examples/rootfs/x86_linux",
            verbose=QL_VERBOSE.DEBUG,
            stdout=pipe,
        )
