    ...
    def test_pe_win_x86_sality(self):
        ...
        def _WriteFile(ql, address, params):
            ...
            if hFile == 0xFFFFFFF5:
                ...
                ql.mem.write(
                    lpNumberOfBytesWritten, ql.pack(nNumberOfBytesToWrite)
                )
            else:
                ...
                ql.mem.write(
                    lpNumberOfBytesWritten, ql.pack32(nNumberOfBytesToWrite)
                )
            return ret
        ...
        def hook_WriteFile(ql, address, params):
            ...
            if hFile == 0x13371337:
                buffer = ql.mem.read(lpBuffer, nNumberOfBytesToWrite)
                try:
                    r, nNumberOfBytesToWrite = ql.amsint32_driver.os.io_Write(
                        buffer
                    )
                    ql.mem.write(
                        lpNumberOfBytesWritten, ql.pack32(nNumberOfBytesToWrite)
        ...
        def hook_StartServiceA(ql, address, params):
            ...
            if service_handle.name == "amsint32":
                if service_handle.name in ql.os.services:
                    ...
                    ql.amsint32_driver = Qiling(
                        [service_path], ql.rootfs, verbose=QL_VERBOSE.DISASM
                    )
                    init_unseen_symbols(
                        ql.amsint32_driver,
        ...
        if ql.amsint32_driver:
            ...
            ql.amsint32_driver.hook_address(
                self.hook_third_stop_address, stop_addr
            )

            # TODO: not sure whether this one is really STDCALL
            ql.amsint32_driver.os.fcall = ql.amsint32_driver.os.fcall_select(
                STDCALL
            )
