...
class EVMTest(unittest.TestCase):
    def test_underflow(self):
        ...
        argu = ql.arch.abi.convert(['uint256'], [20])
        code = code + argu
        ...
        msg0 = ql.arch.create_message(user1, b'', code=code, contract_address=c1)
        ql.run(code=msg0)
        ...
        def check_balance(sender, destination):
            ...
            msg2 = ql.arch.create_message(sender, destination, call_data)
        ...
        call_data = '0xa9059cbb'+ ql.arch.abi.convert(['address'], [user2]) + \
                                        ql.arch.abi.convert(['uint256'], [21])
        msg1 = ql.arch.create_message(user1, c1, call_data)    
