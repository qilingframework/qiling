...
def my_syscall_write(ql, write_fd, write_buf, write_count, *rest):
    if write_fd == 2 and ql.os.fd[2].__class__.__name__ == "ql_pipe":
        ...
        return syscall.ql_syscall_write(
            ql, write_fd, write_buf, write_count, *rest
        )


def my_bind(ql: Qiling):
    params = ql.os.resolve_fcall_params(
        {"fd": UINT, "addr": POINTER, "addrlen": UINT}
    )
    ...
    print(
        Back.GREEN
        + f"Hijack bind({bind_fd}, {bind_addr:#x}, {bind_addrlen})"
        + Back.RESET
    )
    ...
    return (
        0  # from syscall.ql_syscall_bind(ql, bind_fd, bind_addr, bind_addrlen)
    )
...
def my_netgear(path, rootfs):
    ...
    ql.set_api(
        "bind", my_bind, QL_INTERCEPT.ENTER
    )  # intercepting the bind call on enter
