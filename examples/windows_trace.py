...
def spaced_hex(data):
    return b" ".join(
        hexlify(data)[i : i + 2] for i in range(0, len(hexlify(data)), 2)
    ).decode("utf-8")
    ...
    try:
        for i in md.disasm(buf, address):
            return "{:08X}\t{:08X}: {:24s} {:10s} {:16s}".format(
                count[0], i.address, spaced_hex(buf), i.mnemonic, i.op_str
            )
    except:
        import traceback
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r",
        "--reg",
        help="Dump register values with trace option",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-d",
        "--dump",
        help="Directory to dump memory regions to",
        default="dump",
    )
    ...
    for path in args.input:
        ...
        emulate(
            path, args.root, verbose=QL_VERBOSE.DEBUG, enable_trace=args.trace
        )
